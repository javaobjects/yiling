 ...
 */
@Slf4j
@Service("agencyLockFormApplyServiceStrategy")
public class AgencyLockFormApplyServiceStrategy extends AbstractFormStatusHandler {
    @Autowired
    ...
    public Integer getCurrentType() {
        return FormTypeEnum.ENTERPRISE_LOCK.getCode();
    }
    ...
    public Boolean approve(UpdateGBFormInfoRequest request) {
        ...
        ApproveToAgLockRequest formRequest = new ApproveToAgLockRequest();
        formRequest.setId(request.getId());
        Boolean change = agencyLockFormService.approveToAgLock(formRequest);
        if(change){
    ...
    public Boolean reSubmit(UpdateGBFormInfoRequest request) {
        ...
        ResubmitFormRequest submitFormRequest = new ResubmitFormRequest();
        submitFormRequest.setId(request.getId());
        return formService.resubmit(submitFormRequest);
    }
}