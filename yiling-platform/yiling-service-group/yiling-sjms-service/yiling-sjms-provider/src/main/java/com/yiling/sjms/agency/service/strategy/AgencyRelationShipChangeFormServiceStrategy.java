 ...
 */
@Slf4j
@Service("relationShipChangeFormServiceStrategy")
public class AgencyRelationShipChangeFormServiceStrategy extends AbstractFormStatusHandler {
    @Autowired
    FormService formService;

    @Autowired
    AgencyRelationShipChangeFormService relationShipChangeFormService;

    @Override
    public Integer getCurrentType() {
        return FormTypeEnum.ENTERPRISE_REL_SHIP_CHANGE.getCode();
    }
    ...
    public Boolean approve(UpdateGBFormInfoRequest request) {
        ...
        Boolean change = relationShipChangeFormService.approved(agencyLockFormRequest);
        if(change){
