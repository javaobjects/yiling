 ...
 */
@Slf4j
@RocketMqListener(topic = Constants.TOPIC_CHANGE_FORM_REJECT, consumerGroup = Constants.TOPIC_CHANGE_FORM_REJECT)
public class UpdateFormRejectListener extends AbstractMessageListener {
    ...
    protected MqAction consume(String body, MessageExt message, ConsumeConcurrentlyContext context) {
        ...
        log.info("UpdateFormRejectListener MsgId:{}, MQ消费, Topic:{}, Tag:{}，Body:{}", message.getMsgId(), message.getTopic(), message.getTags(), msg);
        UpdateGBFormInfoRequest updateGBFormInfoRequest = JSON.parseObject(msg, UpdateGBFormInfoRequest.class);
        formStrategyContext.taskProgressHandlerMap.get(updateGBFormInfoRequest.getFormType()).reject(updateGBFormInfoRequest);
