...
@Slf4j
@RefreshScope
@RocketMqListener(topic = Constants.TOPIC_HMC_GZH_INTERACT_DELAY, consumerGroup = Constants.TOPIC_HMC_GZH_INTERACT_DELAY)
public class HmcGzhInteractDelayListener extends AbstractMessageListener {
    ...
    protected MqAction consume(String gzhOpenId, MessageExt messageExt, ConsumeConcurrentlyContext context) {
        log.info("[HmcGzhInteractDelayListener]HMC监听器收到消息：{}", gzhOpenId);
        ...
        if (Objects.nonNull(hmcUser)) {
            ...
            if (!redisService.hasKey(health_evaluate_access)) {
                ...
                if (DateUtil.compare(accessDate, gzhUserDTO.getUpdateTime()) > 0) {
                    log.warn("根据redis key 获取到访问记录，key:{},访问时间:{}", health_evaluate_access, accessDate);
                    lotteryFlag = true;
                }
            }
            ...
            if (!redisService.hasKey(lottery_activity_access)) {
                ...
                if (DateUtil.compare(accessDate, gzhUserDTO.getUpdateTime()) > 0) {
                    log.warn("根据redis key 获取到抽奖访问记录，key:{},访问时间:{}", lottery_activity_access, accessDate);
                    evaluateFlag = true;
                }
            }
