...
import com.yiling.export.export.model.ExportOrderEnterpriseBuyerCenterModel;
 ...
 */
@Slf4j
@Service("orderEnterpriseBuyerExportService")
public class OrderEnterpriseBuyerExportServiceImpl implements BaseExportQueryDataService<QueryOrderPageRequest> {
        ...
        try {
            for (int i = 0; i < 2; i++) {
                ...
                do {
                    ...
                    if (i == 0) {
                        writeSheet = EasyExcel.writerSheet(fileIdx, "企业采购订单信息" + sheetNumber).head(ExportOrderEnterpriseBuyerCenterModel.class).build();
                    } else if (i == 1) {
                    ...
                    while (true) {
                        ...
                        if (i == 0) {
                            List<ExportOrderEnterpriseBuyerCenterModel> list = getOrderEnterpriseSell(request);
                            // 写文件
                            if (writeSheet != null) {
    ...
    private List<ExportOrderEnterpriseBuyerCenterModel> getOrderEnterpriseSell(QueryOrderPageRequest request) {
        Page<OrderEnterpriseDTO> page = orderApi.getOrderEnterprisePage(request);
        if (page == null || CollectionUtils.isEmpty(page.getRecords())) {
            return ListUtil.empty();
        }

        List<ExportOrderEnterpriseBuyerCenterModel> list = new ArrayList<>();

        List<Long> ids = page.getRecords().stream().map(order -> order.getId()).collect(Collectors.toList());

        List<Long> eidList = page.getRecords().stream().map(order -> order.getSellerEid()).collect(Collectors.toList());
        ...
        for (OrderEnterpriseDTO one : page.getRecords()) {
            ...
            ExportOrderEnterpriseBuyerCenterModel orderB2BAdmin = new ExportOrderEnterpriseBuyerCenterModel();
            ...
            orderB2BAdmin.setProvinceName(one.getSellerProvinceName());
            orderB2BAdmin.setCityName(one.getSellerCityName());
            orderB2BAdmin.setRegionName(one.getSellerRegionName());
