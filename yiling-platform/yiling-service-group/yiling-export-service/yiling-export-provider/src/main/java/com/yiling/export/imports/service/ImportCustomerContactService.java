package com.yiling.export.imports.service;

import java.io.InputStream;
import java.util.Map;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.yiling.export.excel.model.ImportResultModel;
import com.yiling.export.excel.service.BaseExcelImportService;
import com.yiling.export.excel.util.ExeclImportUtils;
import com.yiling.export.imports.handler.ImportCustomerContactHandler;
import com.yiling.export.imports.model.ImportCustomerContactModel;

import cn.afterturn.easypoi.excel.entity.ImportParams;

/**
 * 导入企业客户商务联系人
 *
 * @author: lun.yu
 * @date: 2023-06-05
 */
@Service
public class ImportCustomerContactService implements BaseExcelImportService {

    @Autowired
    ImportCustomerContactHandler importCustomerContactHandler;

    @Override
    public ImportResultModel importExcel(InputStream inputstream, Integer readRows, Map<String, Object> param) throws Exception {
        ImportParams params = new ImportParams();
        params.setNeedVerify(true);
        params.setVerifyHandler(importCustomerContactHandler);
        params.setKeyIndex(0);
        params.setReadRows(readRows);

        return ExeclImportUtils.importExcelMore(inputstream, ImportCustomerContactModel.class, params, importCustomerContactHandler, param);
    }
}
