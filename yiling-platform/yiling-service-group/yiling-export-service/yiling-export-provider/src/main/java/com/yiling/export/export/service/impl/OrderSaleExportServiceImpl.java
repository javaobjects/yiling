 ...
 * @date:2021/8/21
 */
@Service("orderSaleExportService")
public class OrderSaleExportServiceImpl implements BaseExportQueryDataService<QueryOrderPageRequest> {
   ...
   /* private static final LinkedHashMap<String, String> FIELD = new LinkedHashMap<>();
    ...
    public List<ExportOrderSaleModule> getResult(QueryOrderPageRequest request) {


        Page<OrderDTO> page = orderApi.getOrderPage(request);

        List<ExportOrderSaleModule> result = new ArrayList<>();
        ...
        for (OrderDTO one : page.getRecords()) {
            ...
            ExportOrderSaleModule dataPojo = PojoUtils.map(orderB2BAdmin,ExportOrderSaleModule.class);
            result.add(dataPojo);
        }


        //exportDataDTO.setSheetName("销售订单列表导出");
        // 页签字段
        return result;
    }
    ...
    public byte[] getExportByte(QueryOrderPageRequest request, String fileName) {
        ...
        try {
            ...
            do {
                ...
                writeSheet = EasyExcel.writerSheet(fileIdx, "销售订单列表导出" + sheetNumber).head(ExportOrderSaleModule.class).build();
                while (true) {
                    request.setCurrent(current);
                    request.setSize(size);

                    List<ExportOrderSaleModule> list = getResult(request);
                    // 写文件
                    if (writeSheet != null) {
