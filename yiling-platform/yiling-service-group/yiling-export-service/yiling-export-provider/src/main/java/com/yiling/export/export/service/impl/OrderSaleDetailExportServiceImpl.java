...
import com.yiling.export.export.model.ExportOrderSaleDetailModule;
import com.yiling.export.export.model.ExportOrderSaleModule;
 ...
 * @date:2021/8/21
 */
@Service("orderSaleDetailExportService")
public class OrderSaleDetailExportServiceImpl implements BaseExportQueryDataService<QueryOrderPageRequest> {
   ...
   /* private static final LinkedHashMap<String, String> FIELD = new LinkedHashMap<>();
     ...
     * @return
     */
    public List<ExportOrderSaleDetailModule> getResult(QueryOrderPageRequest request) {
        ...
        List<ExportOrderSaleDetailModule> result = new ArrayList<>();
        ...
        for (OrderDetailDTO detailOne : orderDetailList) {
            ...
            BigDecimal discountAmount = changeDTO.getCouponDiscountAmount().add(changeDTO.getPlatformCouponDiscountAmount()).add(changeDTO.getCashDiscountAmount()).add(changeDTO.getTicketDiscountAmount()).subtract(changeDTO.getSellerPlatformCouponDiscountAmount()).subtract(changeDTO.getSellerCouponDiscountAmount()).subtract(changeDTO.getSellerReturnCashDiscountAmount()).subtract(changeDTO.getSellerReturnTicketDiscountAmount());
            ...
            if (mapDelivery.containsKey(detailOne.getId())) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ExportOrderB2BCenterDetailBO exportOrder = getExportOrderB2BAdminDetailBO(detailOne, orderDTO, enterpriseDTO);
                    exportOrder.setGoodsDiscountRate(goodsDiscountRate);
                    exportOrder.setBatchNo(list.get(i).getBatchNo());
                    exportOrder.setProduceDate(list.get(i).getProduceDate());
                    ...
                    ExportOrderSaleDetailModule dataPojo = PojoUtils.map(exportOrder, ExportOrderSaleDetailModule.class);
                    result.add(dataPojo);
                }
            } else {
                ExportOrderB2BCenterDetailBO exportOrder = getExportOrderB2BAdminDetailBO(detailOne, orderDTO, enterpriseDTO);
        ...
        //exportDataDTO.setSheetName("销售订单明细单明细列表");
        return result;
    }

    private ExportOrderB2BCenterDetailBO getExportOrderB2BAdminDetailBO(OrderDetailDTO detailOne, OrderDTO orderDTO, EnterpriseDTO enterpriseDTO) {
        ...
        exportOrder.setContractNumber(orderDTO.getContractNumber());
        ...
        exportOrder.setGoodsManufacturer(detailOne.getGoodsManufacturer());
        return exportOrder;
    }
    ...
    public byte[] getExportByte(QueryOrderPageRequest request, String fileName) {
        ...
        try {
            ...
            do {
                ...
                writeSheet = EasyExcel.writerSheet(fileIdx, "销售订单明细列表" + sheetNumber).head(ExportOrderSaleDetailModule.class).build();
                while (true) {
                    request.setCurrent(current);
                    request.setSize(size);

                    List<ExportOrderSaleDetailModule> list = getResult(request);
                    // 写文件
                    if (writeSheet != null) {
