...
import com.yiling.export.export.model.ExportB2BOrderSaleModule;
 ...
 * @date:2021/8/21
 */
@Service("orderB2BAdminExportService")
public class OrderB2BAdminExportServiceImpl implements BaseExportQueryDataService<QueryOrderPageRequest> {
     ...
     * @return
     */
    public List<ExportB2BOrderSaleModule> getResult(QueryOrderPageRequest queryOrderPageRequest) {
        //需要返回的对象
        ...
        List<ExportB2BOrderSaleModule> result = new ArrayList<>();
        ...
        for (OrderDTO one : page.getRecords()) {
            ...
            ExportB2BOrderSaleModule dataPojo = PojoUtils.map(orderB2BAdmin, ExportB2BOrderSaleModule.class);
            result.add(dataPojo);
        }


        //exportDataDTO.setSheetName("B2B销售订单列表");
    ...
    public byte[] getExportByte(QueryOrderPageRequest request, String fileName) {
        ...
        try {
            ...
            do {
                ...
                writeSheet = EasyExcel.writerSheet(fileIdx, "B2B销售订单列表" + sheetNumber).head(ExportB2BOrderSaleModule.class).build();
                while (true) {
                    request.setCurrent(current);
                    request.setSize(size);

                    List<ExportB2BOrderSaleModule> list = getResult(request);
                    // 写文件
                    if (writeSheet != null) {
