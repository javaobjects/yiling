...
import com.yiling.export.export.model.ExportB2BOrderPurchaseDetailModule;
 ...
 * @date:2021/8/21
 */
@Service("orderB2BAdminPurchaseDetailExportService")
public class OrderB2BAdminPurchaseDetailExportServiceImpl implements BaseExportQueryDataService<QueryOrderPageRequest> {
   ...
   /* private static final LinkedHashMap<String, String> FIELD = new LinkedHashMap<>();
    ...
    public List<ExportB2BOrderPurchaseDetailModule> getResult(QueryOrderPageRequest queryOrderPageRequest) {
        ...
        List<ExportB2BOrderPurchaseDetailModule> result = new ArrayList<>();
        ...
        for (OrderDetailDTO detailOne : orderDetailList) {
            ...
            BigDecimal discountAmount = changeDTO.getCouponDiscountAmount().add(changeDTO.getPlatformCouponDiscountAmount()).add(changeDTO.getCashDiscountAmount()).add(changeDTO.getTicketDiscountAmount()).add(changeDTO.getPlatformPaymentDiscountAmount()).add(changeDTO.getShopPaymentDiscountAmount()).subtract(changeDTO.getSellerPlatformPaymentDiscountAmount()).subtract(changeDTO.getSellerShopPaymentDiscountAmount()).subtract(changeDTO.getSellerPlatformCouponDiscountAmount()).subtract(changeDTO.getSellerCouponDiscountAmount()).subtract(changeDTO.getSellerReturnCashDiscountAmount()).subtract(changeDTO.getSellerReturnTicketDiscountAmount());
            ...
            if (mapDelivery.containsKey(detailOne.getId())) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    ExportB2BOrderPurchaseDetailModule dataPojo = PojoUtils.map(exportOrder, ExportB2BOrderPurchaseDetailModule.class);
                    result.add(dataPojo);
                }
            } else {
                ExportOrderB2BAdminDetailBO exportOrder = getExportOrderB2BAdminDetailBO(detailOne, orderDTO, enterpriseDTO);
        ...
        //exportDataDTO.setSheetName("B2B采购订单明细列表");
        return result;
    }
    ...
    public byte[] getExportByte(QueryOrderPageRequest request, String fileName) {
        ...
        try {
            ...
            do {
                ...
                writeSheet = EasyExcel.writerSheet(fileIdx, "B2B采购订单明细列表" + sheetNumber).head(ExportB2BOrderPurchaseDetailModule.class).build();
                while (true) {
                    request.setCurrent(current);
                    request.setSize(size);

                    List<ExportB2BOrderPurchaseDetailModule> list = getResult(request);
                    // 写文件
                    if (writeSheet != null) {
