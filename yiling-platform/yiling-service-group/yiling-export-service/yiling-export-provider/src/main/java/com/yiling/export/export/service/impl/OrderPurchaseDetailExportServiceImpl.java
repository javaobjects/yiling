...
import com.yiling.export.export.model.ExportOrderPurchaseDetailModule;
 ...
 * @date:2021/8/21
 */
@Service("orderPurchaseDetailExportService")
public class OrderPurchaseDetailExportServiceImpl implements BaseExportQueryDataService<QueryOrderPageRequest> {
     ...
     * @return
     */
    public List<ExportOrderPurchaseDetailModule> getResult(QueryOrderPageRequest request) {
        ...
        List<ExportOrderPurchaseDetailModule> result = new ArrayList<>();
        ...
        for (OrderDetailDTO detailOne : orderDetailList) {
            ...
            if (mapDelivery.containsKey(detailOne.getId())) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    ExportOrderPurchaseDetailModule dataPojo = PojoUtils.map(exportOrder, ExportOrderPurchaseDetailModule.class);
                    result.add(dataPojo);
                }
            } else {
                ExportOrderB2BCenterDetailBO exportOrder = getExportOrderB2BAdminDetailBO(detailOne, orderDTO, enterpriseDTO);
        ...
        //exportDataDTO.setSheetName("采购订单明细单明细列表");
        return result;
    }
    ...
    public byte[] getExportByte(QueryOrderPageRequest request, String fileName) {
        ...
        try {
            ...
            do {
                ...
                writeSheet = EasyExcel.writerSheet(fileIdx, "采购订单明细单明细列表" + sheetNumber).head(ExportOrderPurchaseDetailModule.class).build();
                while (true) {
                    request.setCurrent(current);
                    request.setSize(size);

                    List<ExportOrderPurchaseDetailModule> list = getResult(request);
                    // 写文件
                    if (writeSheet != null) {
