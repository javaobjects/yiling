 ...
 */
@Slf4j
@RocketMqListener(topic = Constants.TOPIC_REPORT_REWARD_B2B_SEND, consumerGroup = Constants.TAG_REPORT_REWARD_B2B_SEND)
public class B2bReportRewardListener extends AbstractMessageListener {
    ...
    //            }
    //            CreateReportB2bRequest request = JSONObject.parseObject(msg, CreateReportB2bRequest.class);
    //            //校验是否有在修改以岭品关系的任务
    //            List<FlowGoodsRelationEditTaskDTO> reviseTaskList = flowGoodsRelationEditTaskApi.getListByEid(request.getSellerEid()).stream().filter(e->ObjectUtil.equal(e.getSyncStatus(), FlowGoodsRelationEditTaskSyncStatusEnum.WAIT.getCode())).collect(Collectors.toList());
    //            if (CollUtil.isNotEmpty(reviseTaskList)){
    //                ReportTaskDO reportTaskDO=new ReportTaskDO();
    //                //插入报表任务
    //                reportTaskDO.setEid(request.getSellerEid());
    //                reportTaskDO.setParameterJson(msg);
    //                reportTaskDO.setOpTime(new Date());
    //                reportTaskDO.setType(ReportTypeEnum.B2B.getCode());
    ...
    //            }
    //            //校验是否有正在生成报表的任务
    //            List<ReportTaskDO> list = reportTaskService.queryInProductionTask(request.getSellerEid());
    ...
    //
    //            //生成报表
    //            Long reportId = reportService.createB2bReport(request);
    //
    //            //更新报表状态
    ...
    //                }
    //                log.error("MsgId:{}, 应用MQ消费失败, Topic:{}, Tag:{}，Body:{}, 异常信息:{reportId=null}", message.getMsgId(), message.getTopic(), message.getTags(), msg);
    //                return MqAction.ReconsumeLater;
    //            }
    ...
    protected MqAction consume(String body, MessageExt message, ConsumeConcurrentlyContext context) {
        ...
        CreateReportB2bRequest request = JSONObject.parseObject(body, CreateReportB2bRequest.class);
        //校验是否有在修改以岭品关系的任务
        List<FlowGoodsRelationEditTaskDTO> reviseTaskList = flowGoodsRelationEditTaskApi.getListByEid(request.getSellerEid()).stream().filter(e -> ObjectUtil.equal(e.getSyncStatus(), FlowGoodsRelationEditTaskSyncStatusEnum.WAIT.getCode())).collect(Collectors.toList());
        if (CollUtil.isNotEmpty(reviseTaskList)) {
            ReportTaskDO reportTaskDO = new ReportTaskDO();
        ...
        if (ObjectUtil.isNull(reportId)) {
            ...
            log.error("MsgId:{}, 应用MQ消费失败, Topic:{}, Tag:{}，Body:{}, 异常信息:{reportId=null}", message.getMsgId(), message.getTopic(), message.getTags(), body);
            throw new ServiceException(ResultCode.FAILED);
        }
        updatePar.setReportId(reportId);
        updatePar.setStatus(ReportTaskStatusEnum.TASK_SUCCESS.getCode());
