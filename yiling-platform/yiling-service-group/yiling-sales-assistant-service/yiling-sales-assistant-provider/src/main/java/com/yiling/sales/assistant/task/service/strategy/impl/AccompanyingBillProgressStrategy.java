 ...
 */
@Slf4j
@Service("accompanyingBillProgressStrategy")
public class AccompanyingBillProgressStrategy extends AbstractTaskProgressHandler {
    @Autowired
    ...
    public Integer getCurrentType() {
        return FinishTypeEnum.UPLOAD_ACCOMPANYING_BILL.getCode();
    }
    ...
    public Object taskHandle(MarketTaskDO taskDO, UserTaskDO userTask, TaskOrderDO taskOrder, List taskOrderGoodsDOList, Object extraParam) {
        ...
        userTaskGoodsList.stream().filter(u -> orderGoodsIds.contains(u.getGoodsId())).collect(Collectors.toList()).forEach(userTaskGoods -> {
            ...
            int percent = BigDecimal.valueOf(userTaskGoods.getFinishValue()).divide(BigDecimal.valueOf(userTaskGoods.getGoalValue()), 2, BigDecimal.ROUND_HALF_EVEN).multiply(BigDecimal.valueOf(100)).intValue();
            if (percent >= 100) {
                percent = 100;
            }
        ...
        try {
            ...
            this.computeUserTaskProgress(isStepTask, userTask, taskOrder.getRealAmount());
            UserTaskStepDO userTaskStepDO = new UserTaskStepDO();
            //阶梯任务计算完成当前的阶梯
            this.handleStep(taskDO, userTaskStepDO, userTask);

            userTaskService.updateTaskProgress(userTask, userTaskGoodsList, null, null, null, userTaskStepDO);
        } finally {
    ...
    private void computeUserTaskProgress(boolean isStepTask, UserTaskDO userTask, Integer orderTotal) {
        //任务总完数量  从数据库重新查
        ...
        int percent = BigDecimal.valueOf(finishTotal).divide(BigDecimal.valueOf(userTask.getGoal()), 2, BigDecimal.ROUND_HALF_EVEN).multiply(BigDecimal.valueOf(100)).intValue();
        if (percent >= 100) {
            percent = 100;
        }
        String p = String.valueOf(percent);
