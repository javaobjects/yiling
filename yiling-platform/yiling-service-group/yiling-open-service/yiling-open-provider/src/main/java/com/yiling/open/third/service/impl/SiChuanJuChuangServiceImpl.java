 ...
 */
@Slf4j
@Service("siChuanJuChuangService")
public class SiChuanJuChuangServiceImpl extends FlowAbstractTemplate implements BaseFlowInterfaceService {
    ...
    protected String requestPurchaseTab(Map<String, String> param) {
        ...
        paramsMap.put("storageStartTime", startDate);
        paramsMap.put("storageEndTime", endDate);

        // 分页获取数据
        JSONArray dataJsonArray = listPage(appId, flowCardId, secretKey, url, purchaseMethod, paramsMap);
    ...
    protected String requestSaleTab(Map<String, String> param) {
        ...
        Map<String, Object> paramsMap = new HashMap<>();
        paramsMap.put("deliveryStartTime", startDate);
        paramsMap.put("deliveryEndTime", endDate);

        // 分页获取数据
        JSONArray dataJsonArray = listPage(appId, flowCardId, secretKey, url, saleMethod, paramsMap);
        if (ObjectUtil.isNotNull(dataJsonArray) && dataJsonArray.size() > 0) {
            return JSONUtil.toJsonStr(dataJsonArray);
        }
        return null;
    ...
    protected String requestGoodsBatchTab(Map<String, String> param) {
        ...
        // 分页获取数据
        JSONArray dataJsonArray = listPage(appId, flowCardId, secretKey, url, goodsBatchMethod, null);
        if (ObjectUtil.isNotNull(dataJsonArray) && dataJsonArray.size() > 0) {
            return JSONUtil.toJsonStr(dataJsonArray);
        }
        return null;
    ...
    private JSONArray listPage(String appId, String flowCardId, String secretKey, String url, String methodName, Map<String, Object> paramsMap) {
        ...
        do {
            ...
            if (MapUtil.isNotEmpty(paramsMap)) {
                for (String paramKey : paramsMap.keySet()) {
                    map.put(paramKey, paramsMap.get(paramKey));
                }
            }
...
//        String returnJson = "{\"data\":{\"list\":[],\"total\":0},\"msg\":\"success\",\"code\":0}";
        ...
        if (StrUtil.isBlank(returnJson)) {
            log.warn(">>>>> 四川聚创, 查询流向接口异常, response is blank");
            return null;
        }

        JSONObject jsonObject = JSON.parseObject(returnJson);
        if (jsonObject == null) {
            log.warn(">>>>> 四川聚创, 查询流向接口异常, jsonObject is null");
            return new JSONArray();
        }
        Integer code = jsonObject.getInteger("code");
        if (ObjectUtil.isNull(code)) {
            log.warn(">>>>> 四川聚创, 查询流向接口异常, response code is null");
            return null;
        }
        if(code.intValue() != 0){
            String msg = (String)jsonObject.getOrDefault("msg", "");
            log.warn(">>>>> 四川聚创, 查询流向接口异常, code:{}, msg:{}", code, msg);
            return null;
        }

        JSONObject data = jsonObject.getJSONObject("data");
        if (ObjectUtil.isNull(data)) {
            log.warn(">>>>> 四川聚创, 查询流向接口异常, response data is null");
            return null;
        }
        JSONArray jsonArray = data.getJSONArray("list");
        if (jsonArray == null || jsonArray.size() == 0) {
            log.warn(">>>>> 四川聚创, 查询流向接口异常, response data.list is empty");
            return null;
        }
        return new JSONArray();
    }
...
/*
    public static void main(String[] args) {
        String url = "https://sc.api.prod.tyaow.com/open/api/";
...
//        String method = "repertory";
        ...
        long timestamp = System.currentTimeMillis() / 1000;
        String appId = "100037";
        String secretKey = "ea7f7bd6ecc7886cc1d389f66a303871";
        int flowCardId = 235;

        int page = 1;
        int pageSize = 10;
//        String storageStartTime = "2023-06-01 00:00:00";
//        String storageEndTime = "2023-07-30 23:59:59";
        String deliveryStartTime = "2023-06-01 00:00:00";
        String deliveryEndTime = "2023-07-30 23:59:59";
...
//        map.put("storageEndTime", storageEndTime);
        map.put("storageStartTime", deliveryStartTime);
        map.put("storageEndTime", deliveryEndTime);
