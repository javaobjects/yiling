 ...
 */
@Slf4j
@Service("clientOrderPurchaseSendService")
public class ClientOrderPurchaseSendServiceImpl implements SyncTaskService {
    ...
    public void syncData() {
        try {
            ...
            Map<String, String> headParams = pr.generateHeadMap(ErpTopicName.ErpOrderPurchaseSend.getMethod(), initErpConfig.getSysConfig().getKey());
            ...
            //插入到erp系统里
            log.debug("[采购发货单信息同步] 服务端返回信息:" + returnValue);
    ...
    private List<Map<String, String>> insertOrderPurchase(SysConfig sysConfig, String returnObject) {
        ...
        try {
            String yiling_purchase_send = "yiling_purchase_send";
            String yiling_purchase_send_detail = "yiling_purchase_send_detail";
            if (sysConfig.getDbType().equals("Oracle")) {
                yiling_purchase_send = yiling_purchase_send.toUpperCase();
                yiling_purchase_send_detail = yiling_purchase_send_detail.toUpperCase();
            }

            Map<String, String> orderPurchaseList = ErpClientMidTable.getOrderPurchase(sysConfig.getDbType(), yiling_purchase_send);
            Map<String, String> orderPurchaseDetailList = ErpClientMidTable.getOrderPurchaseDetailColumns(sysConfig.getDbType(), yiling_purchase_send_detail);
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                String repeatProcessSql = "select id from yiling_purchase_send where order_id=" + order_id;
                String afterProcessMasterSql = "insert into yiling_purchase_send(";
                String vs = "values(";
                if (sysConfig.getDbType().equals("Oracle")) {
                ...
                for (int j = 0; j < orderDetailObject.size(); j++) {
                    ...
                    String vsD = "values(";
                    String orderItemSqlList = "insert into yiling_purchase_send_detail(";
                    if (sysConfig.getDbType().equals("Oracle")) {
                        vsD = vsD.toUpperCase();
                ...
                log.debug("insert in to yiling_purchase_send, start ...");
                ...
                log.debug("insert in to yiling_purchase_send, id -> {}", id);
...
//            erpLog.setMothedNo(ErpTopicName.ErpOrderPurchaseSend.getMethod());
...
//            popRequest.addErpLog(erpLog, initErpConfig.getSysConfig());
    ...
    public void notificationOrderPurchaseFailure(SysConfig sysConfig, List<Map<String, String>> mapList) {
        PopRequest pr = new PopRequest();
        Map<String, String> headMap = pr.generateHeadMap(ErpTopicName.ErpOrderPurchaseSendReturn.getMethod(), sysConfig.getKey());
