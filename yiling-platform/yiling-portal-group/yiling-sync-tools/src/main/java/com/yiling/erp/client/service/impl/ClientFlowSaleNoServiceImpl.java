 ...
 */
@Slf4j
@Service("clientFlowSaleNoService")
public class ClientFlowSaleNoServiceImpl implements SyncTaskService {
    ...
    public void syncData() {
        try {
            ...
            String dataSql = SqlPaginationUtil.getTopSql("select order_id,sale_no from yiling_order  where  sale_no<>' ' and flow_flag=0 ", sysConfig.getDbType(),10);
            ...
            if (CollUtil.isNotEmpty(mapList)) {
                for (Map<Object, Object> oneMap : mapList) {
                    ...
                    Map<String, String> headMap = pr.generateHeadMap(ErpTopicName.ErpOrderSaleNo.getMethod(), sysConfig.getKey());
                    ...
                    if ((returnValue != null) && (!returnValue.equals(""))) {
                        ...
                        if (200 != code) {
                            log.error("回写销售单号报错{}",JSON.toJSONString(oneMap));
                            continue;
                        }
                        DataBaseConnection.getInstance().updateErpSn(sysConfigPush, "update yiling_order set flow_flag=1 where order_id=" + oneMap.get("order_id"));
                    }
                }
            }
        } catch (Exception e) {
            log.error("回写销售单号报错", e);
        }
    }
}
