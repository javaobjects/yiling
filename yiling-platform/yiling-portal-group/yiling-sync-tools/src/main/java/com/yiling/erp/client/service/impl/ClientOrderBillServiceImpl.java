 ...
 */
@Slf4j
@Service("clientOrderBillService")
public class ClientOrderBillServiceImpl implements SyncTaskService {
    ...
    public void syncData() {
        try {
            ...
            Map<String, String> headParams = pr.generateHeadMap(ErpTopicName.ErpOrderBill.getMethod(), initErpConfig.getSysConfig().getKey());
            ...
            //插入到erp系统里
            log.debug("[出库单转应收单同步] 服务端返回信息"+returnValue);
    ...
    private List<Map<String, String>> insertOrder(SysConfig sysConfig, String returnObject) {
        ...
        try {
            Map<String, String> orderList = ErpClientMidTable.getErpBillColumns(sysConfig.getDbType(), "yiling_bill");
            Map<String, String> orderDetailList = ErpClientMidTable.getOrderBillColumns(sysConfig.getDbType(), "yiling_order_bill");

            for (int i = 0; i < jsonArray.size(); i++) {
                JSONObject object = jsonArray.getJSONObject(i);
                if (StringUtils.isEmpty(object.getString("order_bill_id"))) {
                    return null;
                }

                if (StringUtils.isEmpty(object.getJSONArray("order_bill_list"))) {
                    return null;
                }
                String order_bill_id = object.getString("order_bill_id");
                String eas_delivery_number = object.getString("eas_delivery_number");
                String repeatProcessSql = "select id from yiling_bill where order_bill_id='" + order_bill_id+"'";
                String afterProcessMasterSql = "insert into yiling_bill(";
                ...
                JSONArray orderDetailObject = object.getJSONArray("order_bill_list");
                ...
                for (int j = 0; j < orderDetailObject.size(); j++) {
                    ...
                    String vsD = "values(";
                    String orderItemSqlList = "insert into yiling_order_bill(";
                ...
                if (!StringUtil.isEmpty(id)) {
                    ...
                    resultMap.put(OpenConstants.ORDER_ID, order_bill_id);
                    resultMap.put(OpenConstants.ERP_SN, eas_delivery_number);
                    mapList.add(resultMap);
                }
            }
        } catch (Exception e) {
            log.error("开票申请信息插入到客户端报错", e);
        }
        return mapList;
    }
    ...
    public void notificationOrderIssuedFailure(SysConfig sysConfig, List<Map<String, String>> mapList) {
        PopRequest pr = new PopRequest();
        Map<String, String> headMap = pr.generateHeadMap(ErpTopicName.ErpOrderBillReturn.getMethod(), sysConfig.getKey());
