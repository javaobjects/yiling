...
@Component
@Slf4j
public class StandardHealthImportDataHandler implements ImportDataHandler<StandardHealthImportExcelForm> {
    @DubboReference
    StandardGoodsApi standardGoodsApi;

    @Override
    public List<StandardHealthImportExcelForm> execute(List<StandardHealthImportExcelForm> object, Map<String,Object> paramMap) {
        List<StandardHealthImportExcelForm> ids=new ArrayList<>();
        for (StandardHealthImportExcelForm one:object) {
            try {
                StandardHealthImportExcelRequest request = PojoUtils.map(one, StandardHealthImportExcelRequest.class);
                request.setOpUserId((Long) paramMap.getOrDefault(MyMetaHandler.FIELD_OP_USER_ID,0));
                Long gid = standardGoodsApi.saveStandardHealthAllInfoOne(request);
                if (gid <= 0) {
