...
@Slf4j
@Component
public class ImportParSubGoodsHandler implements IExcelVerifyHandler<ImportParSubGoodsForm>, ImportDataHandler<ImportParSubGoodsForm> {
    ...
    @Override
    public ExcelVerifyHandlerResult verifyHandler(ImportParSubGoodsForm form) {
    ...
    @Override
    public List<ImportParSubGoodsForm> execute(List<ImportParSubGoodsForm> object, Map<String, Object> paramMap) {
        for (ImportParSubGoodsForm form : object) {
            ...
            for (FlowGoodsRelationBO relationBO : relationList) {
                ...
                request.setActivityName(form.getActivityName());
                ...
                if (ObjectUtil.equal(form.getRewardType().toString(), ReportRewardTypeEnum.CASH.getCode().toString())) {
                    request.setRewardAmount(form.getPrice());
                } else if (ObjectUtil.equal(form.getRewardType().toString(), ReportRewardTypeEnum.PERCENTAGE.getCode().toString())) {
                    request.setRewardPercentage(form.getPrice());
                } else {
                    form.setErrorMsg("金额类型不正确");
                    break;
                }
                try {
                    Boolean isSucceed = reportParamApi.saveOrUpdateParamSubActivityGoods(request);
                    if (isSucceed) {
                        succeedList.add(relationBO.getGoodsInSn());
                    }
                } catch (BusinessException be) {
                    String errMsg = buildErrMsg(form.getErrorMsg(), "此以岭id关联的商品内码为{" + relationBO.getGoodsInSn() + "}的商品在参数中已经存在");
