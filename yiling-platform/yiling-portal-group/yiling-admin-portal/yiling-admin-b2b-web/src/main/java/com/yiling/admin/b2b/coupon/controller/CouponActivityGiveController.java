...
import com.yiling.admin.b2b.coupon.from.DeleteCouponActivityEnterpriseGiveRecordFrom;
import com.yiling.admin.b2b.coupon.from.QueryCouponActivityEnterpriseGiveFrom;
import com.yiling.admin.b2b.coupon.from.QueryCouponActivityEnterpriseGiveRecordFrom;
import com.yiling.admin.b2b.coupon.from.SaveCouponActivityEnterpriseGiveRecordFrom;
import com.yiling.admin.b2b.coupon.vo.CouponActivityEnterpriseGivePageVO;
import com.yiling.admin.b2b.coupon.vo.CouponActivityEnterpriseGiveRecordPageVO;
...
public class CouponActivityGiveController extends BaseController {
    ...
    public Result<Page<CouponActivityEnterpriseGivePageVO>> queryEnterpriseListPage(@CurrentUser CurrentAdminInfo adminInfo, @RequestBody QueryCouponActivityEnterpriseGiveFrom from) {
        ...
        if (ObjectUtil.isNotNull(enterpriseDTOPage) && CollUtil.isNotEmpty(enterpriseDTOPage.getRecords())) {
            // 查询已添加的采购商
            Map<Long, CouponDTO> enterpriseInfoMap = new HashMap<>();
            ...
            List<CouponDTO> autoGiveEnterpriseInfoList = couponActivityApi.getHasGiveCountByEids(from.getCouponActivityId(), eidList);
            ...
            for (EnterpriseDTO enterprise : enterpriseDTOPage.getRecords()) {
                ...
                // 已发放数量
                CouponDTO enterpriseInfo = enterpriseInfoMap.get(enterprise.getId());
                int giveNum = 0;
                if(ObjectUtil.isNotNull(enterpriseInfo) && ObjectUtil.isNotNull(enterpriseInfo.getNum())){
                    giveNum = enterpriseInfo.getNum();
                }
    ...
    public Result<Page<CouponActivityEnterpriseGiveRecordPageVO>> queryEnterpriseGiveRecordListPage(@CurrentUser CurrentAdminInfo adminInfo, @RequestBody QueryCouponActivityEnterpriseGiveRecordFrom from) {
    ...
    @Log(title = "优惠券活动-发放",businessType = BusinessTypeEnum.INSERT)
    @ApiOperation(value = "发放-添加供应商进行发券", httpMethod = "POST")
    @PostMapping("/addEnterpriseGiveRecord")
    public Result<Boolean> addEnterpriseGiveRecord(@CurrentUser CurrentAdminInfo adminInfo, @RequestBody @Valid SaveCouponActivityEnterpriseGiveRecordFrom form) {
        ...
        UserDTO user = Optional.ofNullable(userApi.getById(adminInfo.getCurrentUserId())).orElse(new UserDTO());
        request.setOpUserName(user.getName());
        // 企业信息
        request.setOwnEid(0L);
        request.setOwnEname("");
        return Result.success(couponActivityApi.addGiveEnterpriseInfo(request));
    }

    @Log(title = "优惠券活动-删除发放供应商",businessType = BusinessTypeEnum.DELETE)
    @ApiOperation(value = "发放-删除已发券供应商", httpMethod = "POST")
    @PostMapping("/deleteEnterpriseGiveRecord")
    public Result<Boolean> deleteEnterpriseLimit(@CurrentUser CurrentAdminInfo adminInfo, @RequestBody @Valid DeleteCouponActivityEnterpriseGiveRecordFrom form) {
