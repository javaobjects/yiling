package com.yiling.admin.hmc.version.controller;
...
import com.yiling.admin.hmc.version.form.VersionAddForm;
import com.yiling.admin.hmc.version.form.VersionPageForm;
import com.yiling.admin.hmc.version.vo.AppInfoVO;
import com.yiling.admin.hmc.version.vo.AppVersionVO;
...
public class VersionController extends BaseController {
    ...
    public Result<List<AppInfoVO>> pageList(@CurrentUser CurrentAdminInfo staffInfo) {
        List<AppInfoDTO> appInfoDTOList = appVersionApi.listAppInfoByChannelCode(AppChannelEnum.SA_MR);
        List<AppInfoVO> voList = PojoUtils.map(appInfoDTOList, AppInfoVO.class);
        voList.forEach(appInfoVO -> appInfoVO.setChannelCode(AppChannelEnum.SA_MR.getCode()));
    ...
    public Result<Object> saveVersion(@CurrentUser CurrentAdminInfo staffInfo, @RequestBody @Valid VersionAddForm form) {
        ...
        if (!request.getChannelCode().equals(AppChannelEnum.SA_MR.getCode())) {
    ...
    public Result<Page<AppVersionVO>> pageList(@CurrentUser CurrentAdminInfo staffInfo, @RequestBody VersionPageForm form) {
        ...
        request.setChannelCode(AppChannelEnum.SA_MR.getCode());
