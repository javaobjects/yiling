package com.yiling.sales.assistant.app.gateway.filter;
...
import com.yiling.sales.assistant.app.gateway.config.CommonUrlsConfig;
import com.yiling.sales.assistant.app.gateway.config.IgnoreUrlsConfig;
...
public class JwtAuthenticationTokenFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        UserTypeEnum userType = UserTypeEnum.getByCode(data.getUserType());
        Long userId = Convert.toLong(data.getUserId(), 0L);
        Long eid = Convert.toLong(data.getEid(), 0L);

        AppEnum appEnum = AppEnum.getByAppId(data.getAppId());
        if (appEnum == AppEnum.SALES_ASSISTANT_APP) {
            if (userType != UserTypeEnum.ZIRANREN && (userId == 0L || eid == 0L)) {
                log.warn("token信息不全：requestUrl={}, data={}", originalUrl, JSONUtil.toJsonStr(data));
                return this.authError(response, "会话已过期，请重新登录");
            }
        } else if (appEnum == AppEnum.MR_APP) {
            if (userId == 0L || eid == 0L) {
                log.warn("token信息不全：requestUrl={}, data={}", originalUrl, JSONUtil.toJsonStr(data));
                return this.authError(response, "会话已过期，请重新登录");
            }
        ...
        // 验证服务端Token
        boolean verifyServerTokenResult = this.verifyServerToken(appEnum, jwtToken, data.getUserId());
        ...
        // 验证url
        boolean verifyRequestUrlResult = this.verifyRequestUrl(appEnum, data.getUserId(), originalUrl);
     ...
     * @return
     */
    private boolean verifyServerToken(AppEnum appEnum, String jwtToken, Long userId) {
    ...
    private boolean verifyRequestUrl(AppEnum appEnum, Long userId, String requestUrl) {
        if (!requestVerifyUrl) {
            return true;
        }
        ...
        // 用户所有的菜单和按钮
        List<MenuDTO> userMenus = menuApi.listMenusByUser(appEnum == AppEnum.SALES_ASSISTANT_APP ? PermissionAppEnum.SALES_ASSISTANT_APP : PermissionAppEnum.MR_APP, MenuTypeEnum.all(), 0L, userId);
        // 获取配置了url的菜单和按钮
