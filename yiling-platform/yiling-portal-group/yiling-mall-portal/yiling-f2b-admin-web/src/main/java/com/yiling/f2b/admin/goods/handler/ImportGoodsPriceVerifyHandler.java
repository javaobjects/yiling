 ...
 */
@Component
public class ImportGoodsPriceVerifyHandler implements IExcelVerifyHandler<ImportGoodsPriceForm> {
    ...
    @Override
    public ExcelVerifyHandlerResult verifyHandler(ImportGoodsPriceForm obj) {
        ExcelVerifyHandlerResult result = new ExcelVerifyHandlerResult(true);
        // 客户id与客户分组id只能填写一个，都有值报错
        if (obj.getCustomerEid() != null && obj.getCustomerGroupId() != null) {
            result.setSuccess(false);
            result.setMsg("客户id与客户分组id只能填写一个！");
            return result;
        }
        // 具体价格与浮动点位也是唯一都有值报错
        ...
        if (obj.getGoodsId() != null && obj.getGoodsId() != 0) {
            GoodsDTO goodsInfoDTO = goodsApi.queryInfo(obj.getGoodsId());
        ...
        // 判断客户企业ID是否存在
        if (obj.getCustomerEid() != null && obj.getCustomerEid() != 0) {
            EnterpriseDTO customerEnterpriseDto = enterpriseApi.getById(obj.getCustomerEid());
            if (customerEnterpriseDto == null) {
                result.setSuccess(false);
                result.setMsg("客户企业信息不存在");
                return result;
            }
        }
        // 判断客户分组是否存在
