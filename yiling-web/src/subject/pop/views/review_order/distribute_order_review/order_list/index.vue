              ...
              <div class="table-view" :key="$index">
                ...
                <div class="content flex-row-left">
                  ...
                  <div class="content-center font-size-base font-important-color">
                    <div class="item"><span class="font-title-color">合同编号：</span>{{ row.contractNumber }}</div>
                    <div class="item">
                      <span class="font-title-color">支付方式：</span>{{ row.paymentMethod | dictLabel(payType) }}
                    </div>
                    <div class="item">
                      <span class="font-title-color">支付状态：</span>{{ row.paymentStatus | dictLabel(orderPayStatus) }}
                    </div>
                  </div>
                  <div class="content-center font-size-base font-important-color">
                    <div class="item">
                      <span class="font-title-color">货款总金额：</span>{{ row.totalAmount | toThousand('￥') }}
                    </div>
                    <div class="item">
                      <span class="font-title-color">折扣总金额：</span>{{ row.discountAmount | toThousand('￥-') }}
                    </div>
                    <div class="item">
                      <span class="font-title-color">支付总金额：</span>{{ row.paymentAmount | toThousand('￥') }}
                    </div>
                  </div>
                  <div class="content-center-1 font-size-base font-important-color flex1">
                    <div class="item">
                      <span class="font-title-color">购买商品：</span>{{ row.goodsOrderNum || '- -' }}种商品，数量{{ row.goodsOrderPieceNum || '- -' }}
                    </div>
...
<script>
import { getOrderReviewNum, getOrderReviewList } from '@/subject/pop/api/order'
...
export default {
  ...
  computed: {
    ...
    // 审核状态
    orderPreStatus() {
      return orderPreStatus().filter(item => item.value !== 1)
    }
  },
  data() {
    return {
  ...
  methods: {
    async getTotal() {
      let data = await getOrderReviewNum(4)
      if (data) {
        this.totalData = data
      }
    },
    ...
    showDetail(row, type) {
      // 跳转详情
      this.$router.push(`/reviewOrder/distribute_order_detail/${row.id}/${type}`)
    },
    // 搜索部门点击
