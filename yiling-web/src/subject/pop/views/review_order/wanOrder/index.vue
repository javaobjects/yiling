            ...
            <template slot-scope="{ row, $index }">
              <div class="table-view" :key="$index">
                <div class="session">
                  ...
                  <span class="right">审核状态：<span class="col-warn">{{ row.auditStatus | dictLabel(orderPreStatus) }}</span></span>
                </div>
...
export default {
  ...
  methods: {
    async getTotal() {
      let data = await getOrderReviewNum(1)
      if (data) {
        this.totalData = data
      }
    },
    ...
    showDetail(row, type) {
      // 跳转详情
      this.$router.push(`/reviewOrder/wanOrder_detail/${row.id}/${type}`)
    },
    // 搜索部门点击
