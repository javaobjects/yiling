  ...
  <div class="app-container">
    ...
    <div class="app-container-content has-bottom-bar">
      <div class="buy-order-half">
        ...
        <el-row :gutter="8" type="flex">
          ...
          <el-col :span="8">
            <div class="common-box header-box">
              <div class="header-bar">
                <div class="sign"></div>账款信息<span v-if="data.paymentDaysInfo" class="status" :class="[data.paymentDaysInfo.status == 1 ? 'col-down' : 'col-up']">{{ data.paymentDaysInfo.status == 1 ? '启用' : '停用' }}</span>
              </div>
              <div class="box-text">
                <div class="item">
                  <span class="font-title-color" >起止时间：</span>
                  <span v-if="data.paymentDaysInfo.startTime">{{ data.paymentDaysInfo.startTime | formatDate('yyyy.MM.dd') }}-{{ data.paymentDaysInfo.endTime | formatDate('yyyy.MM.dd') }}</span>
                  <span v-else>- -</span>
                </div>
                <div class="item">
                  <span class="font-title-color">信用额度：</span>
                  <span v-if="data.paymentDaysInfo.totalAmount">{{ data.paymentDaysInfo.totalAmount | toThousand('￥') }}</span>
                  <span v-else>- -</span>
                </div>
                <div class="item">
                  <span class="font-title-color">临时额度：</span>
                  <span v-if="data.paymentDaysInfo.temporaryAmount">{{ data.paymentDaysInfo.temporaryAmount | toThousand('￥') }}</span>
                  <span v-else>- -</span>
                </div>
                <div class="item">
                  <span class="font-title-color">已使用额度：</span>
                  <span v-if="data.paymentDaysInfo.usedAmount">{{ data.paymentDaysInfo.usedAmount | toThousand('￥') }}</span>
                  <span v-else>- -</span>
                </div>
                <div class="item">
                  <span class="font-title-color">可使用额度：</span>
                  <span v-if="data.paymentDaysInfo.availableAmount">{{ data.paymentDaysInfo.availableAmount | toThousand('￥') }}</span>
                  <span v-else>- -</span>
                </div>
                <div class="item">
                  <span class="font-title-color">已还款额度：</span>
                  <span v-if="data.paymentDaysInfo.repaymentAmount">{{ data.paymentDaysInfo.repaymentAmount | toThousand('￥') }}</span>
                  <span v-else>- -</span>
                </div>
                <div class="item">
      ...
      <div class="common-box buy-order-table">
        ...
        <!-- <div class="font-title-color font-size-lg mar-l-8 mar-b-16">
          物流信息：{{ data.deliveryType === 1 ? '自有物流' : '第三方物流' }}
        </div> -->
        <div>
    ...
    <yl-dialog
      ...
      <div class="review-dialog">
        <div class="font-size-lg font-important-color mar-b-8">
          <span class="col-up">*</span>反馈详情
        </div>
        <div>
          <el-input
            type="textarea"
  ...
  data() {
    return {
      ...
      navList: [
        ...
        {
          title: '订单审核列表',
          path: '/reviewOrder/reviewOrder_index'
        },
        {
          title: '订单审核详情'
        }
  ...
  methods: {
    ...
    handleReject(type) {
      if (type === 1) {
        ...
        // 通过
        this.$common.confirm('审核通过后订单将推送至EAS进行发货，请确认是否审核通过', r => {
          if (r) {
            this.allow()
          }
        }, null, null, null, '审核通过')
