<template>
  <div class="app-container">
        ...
        <div class="common-box header-box mar-t-16">
          ...
          <div class="box-text">
            ...
            <el-row :gutter="8" type="flex">
              ...
              <el-col :span="8">
                <div class="item">
                  <span class="font-title-color">发票状态：</span >{{ data.invoiceStatus | dictLabel(orderTicketStatus) }}
                </div>
              </el-col>
            </el-row>
            <el-row :gutter="8" type="flex">
              ...
              <el-col :span="8">
                <div class="item">
                  ...
                  <el-select :disabled="isSetRuleCode" v-model="transitionRuleCode" placeholder="转换规则" @change="transitionRuleCodeChange">
                    <el-option v-for="(item, index) in orderInvoiceTransitionRule" :key="item.label + index" :label="item.label" :value="item.value"></el-option>
                  </el-select>
                </div>
              </el-col>
          ...
          <div class="top-view">
            ...
            <div v-show="ticketDiscountFlag">
              <div class="top-view-item mar-b-8" v-for="item in ticketDiscountNoList" :key="item.ticketDiscountNo">
                <div class="scal-20-width">
                  票折单号：<span>{{ item.ticketDiscountNo }}</span>
                </div>
                <div class="scal-20-width">
                  票折金额：<span>￥{{ item.totalAmount }}</span>
                </div>
                <div class="scal-20-width">
                  可用金额：<span>￥{{ item.availableAmount }}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="expand-view" v-for="(deliveryItem, deliveryIndex) in data.orderInvoiceErpDeliveryNoList" :key="deliveryIndex">
              <div class="top-delivery-view border-1px-b mar-b-16">
                <div class="delivery-view-left">
                  <div>
                    <span class="font-title-color">出库单:</span>
                    <span v-if="deliveryItem.erpDeliveryNo">{{ deliveryItem.erpDeliveryNo }}</span>
                  </div>
                </div>
              </div>
              <!-- 商品 -->
              <div class="delivery-goods-view" v-for="(item, index) in deliveryItem.applyInvoiceDetailInfo" :key="index">
                <div class="title flex-row-left">
                  <el-checkbox @change="checkedChange" v-model="item.checked"></el-checkbox ><span class="mar-l-10">{{ item.goodsName }}</span>
                </div>
                <div class="desc-box">
                  ...
                  <div class="right-text box1">
                    ...
                    <div class="text-item">
                      <span class="font-title-color">批准文号：</span>{{ item.goodsLicenseNo }}
                    </div>
                    <div class="text-item">
                      <span class="font-title-color">规格/型号：</span>{{ item.goodsSpecification }}
                    </div>
                  </div>
                  <div class="right-text box2">
                    <div class="text-item"><span class="font-title-color">商品单价：</span>￥{{ item.goodsPrice }}</div>
                    <div class="text-item">
                      <span class="font-title-color">购买数量：</span>{{ item.goodsQuantity }}
                    </div>
                    <div class="text-item">
                  ...
                  <div class="right-text box4" v-show="ticketDiscountFlag">
                    <div class="form-view">
                      ...
                      <el-form class="form" :model="item" :rules="rules" :ref="'form' + deliveryIndex + index">
                        <el-form-item v-show="item.invoiceDiscountType == 1" prop="ticketDiscountRate" inline-message>
                          <el-input
                        ...
                        <el-form-item v-show="item.invoiceDiscountType == 2" prop="ticketDiscountAmount" inline-message>
                          <el-input
                  ...
                  <div class="right-text" v-show="ticketDiscountFlag">
                    <div class="text-item">
                      <span class="font-title-color">商品现折金额：</span> ￥{{ item.cashDiscountAmount }} ({{ item.caseDiscountRate }}%)
                    </div>
                    <div class="text-item">
                      <span class="font-title-color">商品票折金额：</span> ￥{{ item.ticketDiscountAmount }} ({{ item.ticketDiscountRate }}%)
                    </div>
                    <div class="text-item">
                      <span class="font-title-color">商品折扣比率：</span> {{ item.goodsDiscountRate }}%
                    </div>
                  </div>
                </div>
                <!-- 批次 -->
                <div>
                  <el-row
                    ...
                    :gutter="8"
                    type="flex"
                    v-for="(batchItem, batchIndex) in item.batchList"
                    :key="batchIndex"
                  >
                    <el-col :span="5">
                      <div>
                        批次号/序列号：<span class="font-important-color">{{ batchItem.batchNo }}</span>
                      </div>
                    </el-col>
                    <el-col :span="5">
                      <div>
                        生产日期：<span class="font-important-color">{{ batchItem.produceDate | formatDate }}</span>
                      </div>
                    </el-col>
                    <el-col :span="5">
                      <div>
                        有效期至：<span class="font-important-color">{{ batchItem.expiryDate | formatDate }}</span>
                      </div>
                    </el-col>
                    <el-col :span="3">
                      <div>
                        发货数量：<span class="font-important-color">{{ batchItem.deliveryQuantity }}</span>
                      </div>
                    </el-col>
                    <el-col :span="6">
    ...
    <yl-dialog title="选择票折" :visible.sync="showTicket" :destroy-on-close="false" @confirm="confirm">
      <div class="order-ticket-check">
        ...
        >
          ...
          <el-table-column label="票折单号" prop="ticketDiscountNo" align="center"> </el-table-column>
    ...
    <!-- 查看票折 -->
    <yl-dialog title="查看" :width="'65%'" :visible.sync="showTicketOrder" click-right-btn-close>
      <div class="order-ticket-check">
        <div class="mar-b-16">
          <el-row>
        ...
        <yl-table stripe :no-border="false" :show-header="true" :list="ticketOrderList">
    ...
    <!-- 票折行数提示 -->
    <yl-dialog title="提示" :visible.sync="showLineNum" :destroy-on-close="false" @confirm="lineNumConfirm">
...
export default {
  ...
  created() {
    // 搜索框防抖
    this.inputChange = debounce(this.inputChange, 1000)
  },
  mounted() {
  ...
  methods: {
    ...
    submitClick() {
      ...
      // 限额
      if (this.maxAmount && Number(this.totalInvoiceAllAmount) > Number(this.maxAmount)) {
      ...
      this.checkFormValidate()
        .then(async result => {
          ...
          if (this.data.orderInvoiceErpDeliveryNoList && this.data.orderInvoiceErpDeliveryNoList.length > 0) {
            ...
            this.$log(
              this.id,
              this.ticketDiscountFlag,
              ticketDiscountNoList,
              ticketDiscount,
              this.transitionRuleCode,
              this.invoiceSummary,
              erpDeliveryNoList,
              this.invoiceEmail
            )
    ...
    async inputChange(value, deliveryIndex, index, type) {
      ...
      let data = await computeInvoiceAmount(
        this.id,
        item.id,
        item.invoiceDiscountType,
        value,
        deliveryItem.erpDeliveryNo,
        item.goodsPrice,
        batchFormList
      )
      this.$common.hideLoad()
