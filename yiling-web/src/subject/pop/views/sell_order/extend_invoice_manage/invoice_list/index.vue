  ...
  <div class="app-container">
    ...
    <div class="app-container-content">
          ...
          <el-row class="box">
            ...
            <el-col :span="6">
              ...
              <el-select v-model="query.distributorEid" placeholder="请选择销售组织">
                ...
                <el-option v-for="item in mainPartList" :key="item.id" :label="item.name" :value="item.id"></el-option>
              </el-select>
            </el-col>
            <el-col :span="6">
      ...
      <div class="down-box clearfix">
        <div class="btn">
          <ylButton type="primary" v-role-btn="['4']" plain @click="downLoadTemp">导出查询结果</ylButton>
        </div>
      </div>
      <!-- 底部列表 -->
      <div class="mar-t-8 pad-b-10 order-table-view">
        ...
        >
          <el-table-column label-class-name="mar-l-16" label="订单信息" min-width="340" align="left">
            <template slot-scope="{ row }">
              <div class="item text-l mar-l-16">
                ...
                <div class="item-text font-size-base font-title-color">
                  <span>销售组织：</span>{{ row.distributorEname }}
                </div>
          ...
          <el-table-column label="发票状态" min-width="150" align="left">
            <template slot-scope="{ row }">
              <div class="item">
                ...
                <div class="item-text font-size-base font-title-color"></div>
                <div class="item-text font-size-base font-title-color">
                  {{ row.invoiceStatus | dictLabel(orderTicketStatus) }}
                </div>
                <div class="item-text font-size-base font-title-color"></div>
                <div class="item-text font-size-base font-title-color"></div>
              </div>
          ...
          <el-table-column label="操作" min-width="120" align="center">
            <template slot-scope="{ row }">
              <div>
                <yl-button type="text" v-role-btn="['1']" v-show="row.invoiceStatus == 1" @click="editTicketClick(row, 1)" >申请发票</yl-button>
              </div>
              <div>
                <yl-button type="text" v-role-btn="['2']" @click="showOrderTicket(row)">查看发票详情</yl-button>
              </div>
              <div><yl-button type="text" v-role-btn="['3']" @click="showDetail(row, 1)">查看订单详情</yl-button></div>
            </template>
          </el-table-column>
        </yl-table>
      </div>
      <!-- 发票信息 -->
      <yl-dialog title="发票信息" click-right-btn-close :visible.sync="showTicket">
        <div class="order-ticket-check" v-if="ticketInfo.id">
          <div class="order-content">
            <div class="font-size-base font-important-color flex-row-left mar-b-8">
              ...
              <div class="font-title-color flex1">
                <span>发票状态：</span >{{ ticketInfo.status ? $options.filters.dictLabel(ticketInfo.status, orderTicketStatus) : '- -' }}
              </div>
            </div>
            <div class="font-size-base font-important-color flex-row-left mar-b-10">
              <div class="font-title-color flex1">
                <span>发票申请人：</span>{{ ticketInfo.createUserName || '- -' }}
              </div>
              <div class="font-title-color flex1">
                <span>发票申请时间：</span>{{ ticketInfo.createTime | formatDate }}
              </div>
            </div>
          </div>
          <yl-table border :show-header="true" :list="ticketInfo.orderInvoiceInfo">
            <el-table-column label="发票单号" prop="invoiceNo" align="center"> </el-table-column>
          </yl-table>
        </div>
      </yl-dialog>
    </div>
...
<script>
import { getInvoiceList, getAgreementMainPartList } from '@/subject/pop/api/order'
...
export default {
  ...
  computed: {
    // 支付方式
    payType() {
      return paymentMethod().filter(item => item.value != 1)
    },
    // 订单状态
    orderStatus() {
      return orderStatus().filter(item => item.value !== -10 && item.value !== 10 && item.value !== 20 && item.value !== 100)
    },
    // 支付状态
    orderPayStatus() {
