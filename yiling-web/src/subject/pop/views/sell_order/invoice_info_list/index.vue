        ...
        <div class="common-box header-box">
          ...
          <div class="top-view">
            <div class="my-form-item">
              <div class="item-view">发票金额：<span>{{ data.invoiceAmount | toThousand('￥') }}</span> </div>
              <div class="item-view">订单所使用的票折总金额：<span>{{ data.ticketDiscountAmount | toThousand('￥') }}</span> </div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mar-t-8 pad-b-10 order-table-view">
        <yl-table
          ...
          <el-table-column label="发票申请编号" min-width="200" align="center">
            <template slot-scope="{ row }">
              <div><yl-button type="text" @click="showDetail(row)">{{ row.id }}</yl-button></div>
            </template>
          </el-table-column>
