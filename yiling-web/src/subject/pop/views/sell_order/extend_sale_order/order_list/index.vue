    ...
    <div class="app-container-content">
      ...
      <div class="down-box clearfix">
        <div class="btn">
          ...
          <ylButton type="primary" plain v-role-btn="['6']" @click="downLoadTemp1">导出明细</ylButton>
        </div>
      </div>
      <!-- 底部列表 -->
      <div class="mar-t-8 pad-b-10 order-table-view">
        ...
        >
                ...
                <div class="expand-view" v-for="(item, index) in row.goodOrderList" :key="index">
                  ...
                  <div class="desc-box">
                    ...
                    <div class="right-text box2">
                      ...
                      <div class="text-item text-item-red">
                        <span class="font-title-color font-title-red">购买件数：</span >{{ item.goodsQuantity / item.packageNumber }}件
                      </div>
                      <div class="text-item">
                    ...
                    <div class="right-text">
                      ...
                      <div class="text-item">
                        <span class="font-title-color">金额小计：</span>{{ item.goodsAmount | toThousand('￥') }}
                      </div>
                      <div class="text-item">
                        <span class="font-title-color">折扣金额：</span>{{ item.discountAmount | toThousand('￥-') }}
                      </div>
                      <div class="text-item">
                        <span class="font-title-color">支付金额：</span>{{ item.realAmount | toThousand('￥') }}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <template slot-scope="{ row }">
              <div class="item text-l mar-l-16">
                ...
                <div class="item-text font-size-base font-title-color">
                  <span>下单时间：</span>{{ row.createTime | formatDate }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>销售组织：</span>{{ row.sellerEname }}
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="支付信息" min-width="210" align="left">
            <template slot-scope="{ row }">
              <div class="item">
                <div class="title"></div>
                <div class="item-text font-size-base font-title-color">
                  <span>订单状态：</span>{{ row.orderStatus | dictLabel(orderStatus) }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>支付方式：</span>{{ row.paymentMethod | dictLabel(payType) }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>支付状态：</span>{{ row.paymentStatus | dictLabel(orderPayStatus) }}
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="相关金额信息" min-width="240" align="left">
            <template slot-scope="{ row, $index }">
              <div class="item">
                <div class="title"></div>
                <div class="item-text font-size-base font-title-color">
                  <span>货款总金额：</span>{{ row.totalAmount | toThousand('￥') }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>折扣总金额：</span>{{ row.discountAmount | toThousand('￥-') }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>支付总金额：</span>{{ row.paymentAmount | toThousand('￥') }}
                </div>
          ...
          <el-table-column label="商品信息" min-width="330" align="left">
            <template slot-scope="{ row }">
              <div class="item">
                <div class="title"></div>
                <div class="item-text font-size-base font-title-color">
                  <span>购买商品：</span>{{ row.goodsOrderNum || '- -' }}种商品，数量{{ row.goodsOrderPieceNum || '- -' }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>发货商品：</span>{{ row.deliveryOrderNum || '- -' }}种商品，数量{{ row.deliveryOrderPieceNum || '- -' }}
                </div>
                <div class="item-text font-size-base font-title-color">
                  <span>签收商品：</span>{{ row.receiveOrderNum || '- -' }}种商品，数量{{ row.receiveOrderPieceNum || '- -' }}
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="操作" min-width="120" align="left">
            <template slot-scope="{ row }">
              <div><yl-button type="text" v-role-btn="['1']" @click="showDetail(row, 1)">查看详情</yl-button></div>
              <div>
                <yl-button type="text" v-role-btn="['2']" v-if="row.judgeDeliveryFlag" @click="showDetail(row, 2)">发货</yl-button>
              </div>
              <div>
                <yl-button type="text" v-if="row.judgeCancelFlag" v-role-btn="['3']" @click="cancleDidClick(row)">取消订单</yl-button>
              </div>
              <div>
                <yl-button type="text" v-role-btn="['4']" v-show="row.orderReturnFlag" @click="showDetail(row, 3)">查看退货详情</yl-button>
              </div>
            </template>
          </el-table-column>
        </yl-table>
...
<script>
import { getOrderList, orderCancle } from '@/subject/pop/api/order'
...
export default {
  ...
  computed: {
    ...
    // 订单状态
    orderStatus() {
      return orderStatus().filter(item => item.value !== 10 && item.value !== 100)
    },
    // 支付状态
    orderPayStatus() {
