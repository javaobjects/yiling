  ...
  export default {
    ...
    data() {
      return {
        ...
        returnType: null,
        rules: {
          // produceDate: [{ required: true, message: '请输入生产期', trigger: 'change' }],
      ...
      receiveThePackage() {
        ...
        this.checkFormValidate().then(async (result) => {
          ...
          if (this.data.orderDetailDelivery && this.data.orderDetailDelivery.length) {
            ...
            this.$common.showLoad()
            let data = await orderDelivery(this.data.id, this.deliveryType, array, this.deliveryCompany, this.deliveryNo)
            this.$common.hideLoad()
            if (data && data.result) {
