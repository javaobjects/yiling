      ...
      <div class="mar-t-8 pad-b-10 order-table-view">
        <yl-table
          ...
          <el-table-column label="退款信息" min-width="284" align="left">
            <template slot-scope="{ row }">
              <div class="item">
                ...
                <div class="item-text font-size-base font-title-color"><span>退款金额：</span>￥{{ row.returnAmount }}</div>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" min-width="120" align="center">
            <template slot-scope="{ row }">
              <div v-role-btn="['4']"><yl-button type="text" @click="showDetail(row)">查看详情</yl-button></div>
              <div v-role-btn="['1']" v-show="row.isAllowCheck == 1"><yl-button type="text" @click="auditClick(row)">审核</yl-button></div>
            </template>
          </el-table-column>
        </yl-table>
      </div>
