  ...
  <div class="app-container">
    ...
    <div class="app-container-content has-bottom-bar">
      ...
      <div class="top-box">
        ...
        <div class="detail-item">
          <yl-button type="text" @click="recordVisibleDidClick()">修改详情</yl-button>
        </div>
      </div>
      <!-- 返利条件设置 -->
      <div class="top-box" v-if="agreementInfo.type == 1">
        ...
        <div class="detail-item">
          ...
          <div class="detail-item-right">
            <div v-if="agreementInfo.agreementsCondition.payType == 0">不指定</div>
        ...
        <div class="detail-item">
          ...
          <div class="detail-item-right">
            <div v-if="agreementInfo.agreementsCondition.backAmountType == 0">不指定</div>
      ...
      <div class="top-box">
        ...
        <div class="font-size-base font-title-color item-text" >
          <div v-if="agreementInfo.restitutionType == 0">不指定</div>
    ...
    <yl-dialog
      ...
      >
        <div class="record-content">
          <div style="margin-bottom:10px;">修改日志详情</div>
          <div v-for="(item, index) in agreementsSnapshotList" :key="index">
            <yl-button type="text" @click="agreementsSnapshotClick(item)">版本：{{ item.version }}</yl-button>
          </div>
        </div>
    </yl-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    recordVisibleDidClick() {
      this.getAgreementAndSnapshotDetailMethod(() => {
        this.recordVisible = true
      })
    },
    async getAgreementAndSnapshotDetailMethod(callback) {
      this.loading = true;
      let params = this.params
      let data = await getAgreementAndSnapshotDetail(
        params.supplementId
      );
      this.loading = false;
      if (data && data.agreementsSnapshotList && data.agreementsSnapshotList.length > 0) {
        this.agreementsSnapshotList = data.agreementsSnapshotList;
        if (callback) callback()
      } else {
        this.$common.warn('暂无修改详情')
      } 
    },
    // 协议版本点击
    agreementsSnapshotClick(item) {
      this.$common.log(item)
      this.$router.push({
        name: 'SupplementDetailRecord',
        params: { supplementId: item.agreementId, agreementSnapshotId: item.id }
      });
    }
