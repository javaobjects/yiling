  ...
  <div class="app-container">
    ...
    <div class="app-container-content">
        ...
        <div class="search-box">
          <el-row class="box">
            ...
            <el-col :span="6">
              ...
              <el-select v-model="query.activityStatus" placeholder="请选择活动类型">
                <el-option label="全部" :value="0"></el-option>
                <el-option
      ...
      <div class="down-box clearfix">
        <div class="btn">
          <ylButton v-role-btn="['1']" type="primary" @click="addClick">添加优惠券</ylButton>
          <ylButton v-role-btn="['9']" type="primary" plain @click="listDownLoadTemp">导出查询结果</ylButton>
        </div>
      </div>
      <!-- 底部列表 -->
        ...
        <yl-table
          ...
          <el-table-column label-class-name="mar-l-16" label="优惠券信息" min-width="350" align="left">
            <template slot-scope="{ row }">
              <div class="item text-l mar-l-16">
                ...
                <div class="multi-text font-size-base font-title-color"><span>优惠规则：</span>{{ row.couponRules }}</div>
          ...
          <el-table-column label="支付信息" min-width="300" align="left">
            <template slot-scope="{ row }">
              <div class="item">
                <div class="title"></div>
                <div class="item-text font-size-base font-title-color"><span>是否自助领取：</span>{{ row.sponsorType | dictLabel(sponsorTypeArray) }}</div>
                <div class="item-text font-size-base font-title-color"><span>优惠券状态：</span>{{ row.status | dictLabel(statusArray) }}</div>
                <div v-if="row.useDateType == 1" class="item-text font-size-base font-title-color"><span>有效期：</span>{{ row.beginTime | formatDate }} - {{ row.endTime | formatDate }}</div>
                <div v-else-if="row.useDateType == 2" class="item-text font-size-base font-title-color"><span>有效期：</span>{{ row.giveOutEffectiveRules }}</div>
            ...
            <template slot-scope="{ row }">
              <div class="operation-view">
                <div class="option">
                  ...
                  <yl-button v-role-btn="['3']" type="text" :disabled="!row.updateFlag" @click="editClick(row)">修改</yl-button>
                  <yl-button v-role-btn="['4']" type="text" :disabled="!row.increaseFlag" @click="addCouponClick(row)">增券</yl-button>
                  <yl-button v-role-btn="['5']" type="text" :disabled="!row.giveFlag" @click="sendClick(row)">发放</yl-button>
                  <yl-button v-role-btn="['6']" type="text" :disabled="!row.copyFlag" @click="copyClick(row)">复制</yl-button>
                  <yl-button v-role-btn="['7']" type="text" :disabled="!row.stopFlag" @click="stopClick(row)">停用</yl-button>
                  <yl-button v-role-btn="['8']" type="text" :disabled="!row.scrapFlag" @click="cancellationClick(row)">作废</yl-button>
                </div>
              </div>
            </template>
          </el-table-column>
    ...
    <!-- 已发放数量 -->
    <yl-dialog title="已发放优惠券查看" :visible.sync="sendNumDialog" width="980px" :show-footer="false">
      <div class="dialog-content-box">
        ...
        <div class="mar-t-10" style="padding-bottom: 10px;background: #FFFFFF;">
          <yl-table
            ...
            @getList="getSendList"
          >
            <el-table-column label="优惠券ID" min-width="100" align="center" prop="id">
            </el-table-column>
            <el-table-column label="优惠规则" min-width="120" align="center" prop="couponRules">
            </el-table-column>
            ...
            <el-table-column label="有效期信息" min-width="200" align="center">
              <template slot-scope="{ row }">
            ...
            <el-table-column label="发放人" min-width="120" align="center" prop="getUserName" fixed="right">
            </el-table-column>
          </yl-table>
        </div>
      </div>
    ...
    <!-- 已使用数量 -->
    <yl-dialog title="已使用优惠券查看" :visible.sync="usedNumDialog" width="980px" :show-footer="false">
      <div class="dialog-content-box">
        <div>
        ...
        <div class="mar-t-10" style="padding-bottom: 10px;background: #FFFFFF;">
          <yl-table
            ...
            @getList="getUsedList"
          >
            <el-table-column label="优惠券ID" min-width="100" align="center" prop="couponId">
            </el-table-column>
            <el-table-column label="优惠规则" min-width="120" align="center" prop="couponRules">
            </el-table-column>
            <el-table-column label="获券企业ID" min-width="100" align="center" prop="eid">
            ...
            <el-table-column label="优惠金额" min-width="100" align="center" prop="amount">
            </el-table-column>
            <el-table-column label="订单金额" min-width="120" align="center" prop="orderAmount">
            </el-table-column>
          </yl-table>
        </div>
      </div>
...
import { couponActivityQueryListPage, couponActivityCopy, couponActivityIncrease, couponActivityScrap, couponActivityStop, queryGiveListPage, queryUseListPage } from '@/subject/pop/api/b2b_api/discount_coupon'
import { statusArray, couponTypeArray, getTypeArray, activityStatusArray } from '../discount_coupon_dict'
import { onInputLimit } from '@/common/utils'
import AddSendDialog from '../add_send_dialog';
...
export default {
  ...
  activated() {
    this.getList()
    console.log('currentEnterpriseInfo:', this.currentEnterpriseInfo)
  },
  methods: {
    async getList() {
      this.loading = true
      let query = this.query
      let data = await couponActivityQueryListPage(
        query.page,
        query.limit,
        query.name,
        query.id,
    ...
    addCouponConfirm() {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          ...
          let data = await couponActivityIncrease(addForm.id, addForm.quantity, addForm.remark)
    ...
    async copyClick(row) {
      this.$common.showLoad()
      let data = await couponActivityCopy(row.id)
      this.$common.hideLoad()
      if (data) {
    ...
    async getSendList() {
      ...
      let data = await queryGiveListPage(
        sendNumQuery.page,
        sendNumQuery.limit,
        this.currentRow.id
      );
    ...
    async getUsedList() {
      ...
      let data = await queryUseListPage(
        usedNumQuery.page,
        usedNumQuery.limit,
        this.currentRow.id
      );
    ...
    async stopConfirm() {
      this.$common.showLoad()
      let data = await couponActivityStop(this.currentOperationRow.id)
      this.$common.hideLoad()
      if (typeof data != 'undefined') {
    ...
    async cancellationConfirm() {
      this.$common.showLoad()
      let data = await couponActivityScrap(this.currentOperationRow.id)
      this.$common.hideLoad()
      if (typeof data != 'undefined') {
    ...
    // 导出已发放优惠券
    async listDownLoadTemp() {
      let query = this.query
      this.$common.showLoad()
      let data = await createDownLoad({
        className: 'CouponActivityInfoExportService',
        fileName: '优惠券信息导出',
        groupName: '商家后台优惠券信息',
        menuName: '商家后台优惠券信息',
        searchConditionList: [
          {
            desc: '企业ID',
            name: 'eid',
            value: this.currentEnterpriseInfo.id
          },
          {
            desc: '优惠券名称',
            name: 'name',
    ...
    async downLoadUsedTemp() {
      this.$common.showLoad()
      let data = await createDownLoad({
        className: 'couponActivityHasUsedExportService',
        fileName: '优惠券活动已使用导出',
        groupName: '优惠券活动',
        menuName: '优惠券管理-优惠券活动',
        searchConditionList: [
