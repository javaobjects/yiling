<template>
  <!-- 添加商品弹框 -->
  <yl-dialog title="设置用户" :visible.sync="addGoodsDialog" width="966px" :show-footer="false">
    <div class="dialog-content-box-customer">
      <div class="dialog-content-top">
        <div class="title-view">
          <div class="sign"></div>选择需添加用户
        </div>
        <div class="search-box">
          <el-row class="box">
            <el-col :span="8">
              <div class="title">商家名称</div>
              <el-input v-model="goodsQuery.ename" placeholder="请输入商家名称" @keyup.enter.native="goodsHandleSearch" />
            </el-col>
          </el-row>
        </div>
        <div class="search-box">
      ...
      <div class="dialog-content-top">
        <div class="title-view">
          <div class="sign"></div>已添加用户
        </div>
        <div class="search-box">
          <el-row class="box">
            <el-col :span="8">
              <div class="title">商家名称</div>
              <el-input v-model="bottomGoodsQuery.ename" placeholder="请输入商家名称" @keyup.enter.native="bottomGoodsHandleSearch" />
            </el-col>
          </el-row>
        </div>
        <div class="search-box">
...
<script>
import { queryEnterpriseListPage, queryEnterpriseLimitListPage, saveEnterpriseLimit, deleteEnterpriseLimit } from '@/subject/pop/api/b2b_api/discount_coupon'
...
export default {
  ...
  methods: {
    ...
    async addClick(row) {
      ...
      let data = await saveEnterpriseLimit(enterpriseLimitList)
      if (data) {
    ...
    async removeClick(row) {
      let ids = []
      ids.push(row.id)
      let data = await deleteEnterpriseLimit(this.couponActivityId, ids)
      if (data) {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
