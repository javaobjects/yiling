...
<!--                <div class="item"><span class="font-title-color">订单状态：</span>{{ data.orderStatus | dictLabel(orderStatus) }}</div>-->
      ...
      <div class="common-box buy-order-table">
        ...
        <div class="font-title-color font-size-lg mar-l-8 mar-b-16 deliver-info" v-show="deliveryType == 2">
          ...
          <el-input v-if="deliveryCompany == '自定义'" v-model="customDeliveryCompany" placeholder="快递名称" />
                  ...
                  <el-form
                    ...
                    <div class="content-item flex-row-left font-title-color">
                      ...
                      <el-form-item
                        prop="expiryDate"
                        :rules="{
                          required: order.deliveryQuantity != 0, message: '请输入有效期', trigger: 'change'
                        }"
                        inline-message>
                        <el-date-picker
                          format="yyyy/MM/dd"
                    ...
                    <div class="content-item flex-row-left font-title-color">
                      ...
                      <el-form-item
                        prop="batchNo"
                        :rules="{
                          required: order.deliveryQuantity != 0, message: '请输入批次号', trigger: 'blur'
                        }"
                        inline-message>
                        <el-input
                          v-model="order.batchNo"
                          :min="0"
                          placeholder="批次号/序列号" />
                      </el-form-item>
                    </div>
                    <div class="content-item flex-row-left font-title-color">
                      ...
                      <el-form-item
                        ...
                        <el-input
                          ...
                          :min="0"
                          @input="e => (order.deliveryQuantity = checkReceiveQuantity(e, item.goodsQuantity, index, idx))"
                          placeholder="请输入发货数量" />
                      </el-form-item>
                    </div>
                    <div class="flex-row-left">
                      <svg-icon class="batch-add" v-if="idx == 0" :icon-class="'batch_add'" @click="batchAddClick(index)" />
                      <svg-icon class="batch-add" v-else :icon-class="'batch_subtract'" @click="batchSubtractClick(index)" />
                    </div>
                  </el-form>
                </div>
              </div>
...
<script>
  import { getSellOrderDetail, orderDelivery } from '@/subject/pop/api/b2b_api/sell_order'
  import { paymentMethod, orderStatus, orderPayStatus, orderTicketStatus, orderReturnStatus, orderDeliveryCompany } from '@/subject/pop/utils/busi'
  ...
  export default {
    ...
    methods: {
      ...
      receiveThePackage() {
        ...
        if (this.deliveryType == 2) { // 第三方物流
          if (this.deliveryCompany == ''){
            this.$common.warn('请先选择快递公司')
            return false
          }
          if (this.deliveryCompany == '自定义' && this.customDeliveryCompany == ''){
            this.$common.warn('请先填写快递名称')
            return false
          }
          if (this.deliveryNo == ''){
        ...
        this.checkFormValidate().then(async (result) => {
          ...
          if (this.data.orderDetailDelivery && this.data.orderDetailDelivery.length) {
            ...
            if (this.deliveryCompany == '自定义') {
              deliveryCompanyName = this.customDeliveryCompany
            } else {
              deliveryCompanyName = this.deliveryCompany
            }
            this.$common.showLoad()
            let data = await orderDelivery(this.data.id, this.deliveryType, array, deliveryCompanyName, this.deliveryNo)
            this.$common.hideLoad()
