...
<script>
import { strategyLimitMemberPage, presaleMemberLimitMemberPageList, presaleMemberLimitMemberAdd, presaleMemberLimitMemberDelete } from '@/subject/admin/api/view_marketing/goods_presell'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let data = await strategyLimitMemberPage(
        goodsQuery.page,
        goodsQuery.limit
      );
      this.loading1 = false
    ...
    async getBottomList() {
      ...
      let data = await presaleMemberLimitMemberPageList(
    ...
    async addClick(row) {
      let data = await presaleMemberLimitMemberAdd(this.marketingPresaleId, row.id)
    ...
    async removeClick(row) {
      let data = await presaleMemberLimitMemberDelete(row.id)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
