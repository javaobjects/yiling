...
<script>
import { strategyLimitBuyerPage, presaleBuyerLimitBuyerPageList, presaleBuyerLimitBuyerAdd, presaleBuyerLimitBuyerDelete } from '@/subject/admin/api/view_marketing/goods_presell'
...
export default {
  ...
  methods: {
    init(marketingPresaleId, running, operationType) {
      this.marketingPresaleId = marketingPresaleId
    ...
    async getList() {
      ...
      let data = await strategyLimitBuyerPage(
        goodsQuery.page,
        goodsQuery.limit,
        goodsQuery.eid,
        goodsQuery.ename
    ...
    async getBottomList() {
      ...
      let data = await presaleBuyerLimitBuyerPageList(
        bottomGoodsQuery.page,
        bottomGoodsQuery.limit,
        this.marketingPresaleId,
    ...
    async addClick(row) {
      let eidList = []
      eidList.push(row.id)
      let data = await presaleBuyerLimitBuyerAdd(this.marketingPresaleId, eidList)
    ...
    async addSearchClick() {
      ...
      let data = await presaleBuyerLimitBuyerAdd(this.marketingPresaleId, undefined, goodsQuery.eid, goodsQuery.ename)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
    ...
    async removeClick(row) {
      let ids = []
      ids.push(row.eid)
      let data = await presaleBuyerLimitBuyerDelete(this.marketingPresaleId, ids)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
    ...
    async deleteSearchClick() {
      ...
      let data = await presaleBuyerLimitBuyerDelete(this.marketingPresaleId, undefined, bottomGoodsQuery.eid, bottomGoodsQuery.ename)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
