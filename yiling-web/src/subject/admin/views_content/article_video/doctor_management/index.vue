  ...
  <div class="app-container">
    ...
    <div class="app-container-content">
              ...
              <div>
                <el-select class="select-width" v-model="query.moduleId" placeholder="请选择" @change="plateChange">
                  ...
                  <el-option
                    ...
                    :key="item.moduleId"
                    :label="item.moduleId | dictLabel(ihModuleData)"
                    :value="item.moduleId">
                  </el-option>
                </el-select>
              </div>
      ...
      <div class="mar-t-8 order-table-view">
        <yl-table
          ...
          <el-table-column align="center" min-width="120" label="所属板块">
            <template slot-scope="{ row }">
              {{ row.moduleId | dictLabel(ihModuleData) }}
            </template>
          </el-table-column>
          <el-table-column align="center" min-width="120" label="所属栏目" prop="categoryName"></el-table-column>
          <el-table-column align="center" min-width="90" label="内容类别">
            <template slot-scope="{ row }">
              {{ row.contentType == 1 ? '文章' : '视频' }}
            </template>
          </el-table-column>
          <el-table-column align="center" min-width="90" label="浏览量(pv)" prop="view"></el-table-column>
          <el-table-column align="center" min-width="100" label="栏目列表排序" prop="categoryRank"></el-table-column>
          <el-table-column align="center" min-width="90" label="内容所属医生" prop="docName"></el-table-column>
    ...
    <yl-dialog 
      ...
      <div class="edit-dialog">
        内容权限：
        <el-radio-group v-model="editData.radio">
...
<script>
import { queryDoctorByNameAndHospital, updateReferStatus, updateIHDocContentAuth } from '@/subject/admin/api/content_api/article_video'
import { displayLine } from '@/subject/admin/utils/busi'
import { doctorContentPage, doctorContentRank, getModulesByLineId } from '@/subject/admin/api/content_api/multiterminal'
import { ihModule, cmsContentCreateSource } from '@/subject/admin/busi/content/article_video'
...
export default {
  ...
  methods: {
    ...
    // 获取所属栏目
    async getColumn() {
      let data = await getModulesByLineId(2)
      if (data) {
    ...
    async getList() {
      ...
      this.loading = true;
      let data = await doctorContentPage(
        query.title,
        query.moduleId,
        query.categoryId,
        query.contentType,
    ...
    isTopClick(row) {
      ...
      .then( async() => {
        this.$common.showLoad();
        let data = await doctorContentRank(
          row.categoryRank,
          row.id,
          row.topFlag == 1 ? 0 : 1
        )
    ...
    referStatusClick(row) {
      ...
      .then( async() => {
        this.$common.showLoad();
        let data = await updateReferStatus(
          row.id,
    ...
    async editConfirm() {
      this.$common.showLoad();
      let data = await updateIHDocContentAuth(
          this.editData.id,
          this.editData.radio
        )
        this.$common.hideLoad();
        if (data !== undefined) {
          this.$common.n_success('编辑成功!');
          this.getList();
        }
