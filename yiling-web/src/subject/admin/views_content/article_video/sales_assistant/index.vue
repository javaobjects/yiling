              ...
              <div>
                <el-select class="select-width" v-model="query.moduleId" placeholder="请选择" @change="plateChange">
                  ...
                  <el-option
                    ...
                    :key="item.moduleId"
                    :label="item.moduleId | dictLabel(saModuleData)"
                    :value="item.moduleId">
                  </el-option>
                </el-select>
              </div>
        ...
        <yl-table
          ...
          <el-table-column align="center" min-width="150" label="所属板块">
            <template slot-scope="{ row }">
              {{ row.moduleId | dictLabel(saModuleData) }}
            </template>
          </el-table-column>
...
import { zsQueryContentPage, zsContentRank, getModulesByLineId, zsUpdateReferStatus } from '@/subject/admin/api/content_api/multiterminal'
import { saModule, cmsContentCreateSource } from '@/subject/admin/busi/content/article_video'
export default {
  name: 'SalesAssistant',
  components: {
  },
  ...
  methods: {
    ...
    // 获取所属板块以及栏目
    async getColumn() {
      let data = await getModulesByLineId(6)
      if (data) {
    ...
    async getList() {
      ...
      this.loading = true;
      let data = await zsQueryContentPage(
        query.title,
        query.moduleId,
        query.categoryId,
        query.contentType,
    ...
    referStatusClick(row) {
      ...
      .then( async() => {
        this.$common.showLoad();
        let data = await zsUpdateReferStatus(
          row.id,
