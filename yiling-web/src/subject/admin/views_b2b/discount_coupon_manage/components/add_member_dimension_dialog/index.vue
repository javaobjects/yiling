...
<script>
import { queryMemberCouponRelationList, autoGetqueryMemberCouponRelationLimit, autoGetSaveMemberCouponRelation, autoGetDeleteMemberCouponRelation } from '@/subject/admin/api/b2b_api/discount_coupon'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      this.loading1 = true
      let data = await queryMemberCouponRelationList(
    ...
    async getBottomList() {
      this.loading2 = true
      let data = await autoGetqueryMemberCouponRelationLimit(
        this.couponActivityId
      );
      this.loading2 = false
      if (data) {
    ...
    async addClick(row) {
      let ids = []
      ids.push(row.id)
      let data = await autoGetSaveMemberCouponRelation(this.couponActivityId, ids)
      if (data) {
    ...
    async removeClick(row) {
      let ids = []
      ids.push(row.id)
      let data = await autoGetDeleteMemberCouponRelation(ids)
      if (data) {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
