...
<script>
import { queryGoodsListPage, queryGoodsLimitListPage, saveGoodsLimit, deleteGoodsLimit, saveGoodsLimitSearchResult, deleteGoodsLimitSearchResult } from '@/subject/admin/api/b2b_api/discount_coupon'
...
export default {
  ...
  methods: {
    init(couponActivityId, enterpriseLimit, running, operationType) {
    ...
    async addClick(row) {
      ...
      let data = await saveGoodsLimit(this.enterpriseLimit, enterpriseLimitList)
      if (data) {
    ...
    async addSearchClick() {
      ...
      let data = await saveGoodsLimitSearchResult(this.couponActivityId, this.enterpriseLimit, goodsQuery.goodsName, Number(goodsQuery.goodsId), goodsQuery.ename)
      if (data) {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
    ...
    async deleteSearchClick() {
      ...
      let data = await deleteGoodsLimitSearchResult(this.couponActivityId, this.enterpriseLimit, bottomGoodsQuery.goodsName, Number(bottomGoodsQuery.goodsId), bottomGoodsQuery.ename)
      if (data) {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
