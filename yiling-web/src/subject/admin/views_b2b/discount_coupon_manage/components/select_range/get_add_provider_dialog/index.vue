...
<script>
import { autoGetQueryEnterpriseListPage, autoGetQueryMemberLimitListPage, autoGetSaveEnterpriseLimit, autoGetDeleteEnterpriseLimit } from '@/subject/admin/api/b2b_api/discount_coupon'

export default {
  // 指定客户
  name: 'GetAddProviderDialog',
  components: {},
  computed: {
  },
  data() {
  ...
  methods: {
    ...
    async getList() {
      ...
      let data = await autoGetQueryEnterpriseListPage(
        goodsQuery.page,
        goodsQuery.limit,
    ...
    async getBottomList() {
      ...
      let data = await autoGetQueryMemberLimitListPage(
    ...
    async addClick(row) {
      ...
      let data = await autoGetSaveEnterpriseLimit(enterpriseLimitList)
      if (data) {
    ...
    async removeClick(row) {
      let ids = []
      ids.push(row.id)
      let data = await autoGetDeleteEnterpriseLimit(this.couponActivityId, ids)
      if (data) {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
