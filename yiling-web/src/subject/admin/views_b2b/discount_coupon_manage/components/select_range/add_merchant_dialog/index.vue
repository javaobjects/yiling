    ...
    <div class="dialog-content-box-customer">
      <div class="dialog-content-top">
        <div class="title-view">
          <div class="sign"></div>选择需添加商家
        </div>
        <div class="search-box">
          <el-row class="box">
            <el-col :span="8">
              <div class="title">企业ID</div>
              <el-input v-model="goodsQuery.eid" placeholder="请输入企业ID" />
            </el-col>
            <el-col :span="8">
              <div class="title">企业名称</div>
              <el-input v-model="goodsQuery.ename" placeholder="请输入企业名称" />
            </el-col>
            <el-col :span="8">
      ...
      <div class="dialog-content-top">
        <div class="title-view">
          <div class="sign"></div>已添加商家
        </div>
        <div class="search-box">
          <el-row class="box">
            <el-col :span="8">
              <div class="title">企业ID</div>
              <el-input v-model="bottomGoodsQuery.eid" placeholder="请输入企业ID" />
            </el-col>
            <el-col :span="8">
              <div class="title">企业名称</div>
              <el-input v-model="bottomGoodsQuery.ename" placeholder="请输入企业名称" />
            </el-col>
            <el-col :span="8">
              <div class="title">企业所属区域</div>
...
<script>
import { autoGetQueryPromotionEnteroriseListPage, queryPromotionListPage, savePromotionEnterpriseLimit, deletePromotionEnterpriseLimit } from '@/subject/admin/api/b2b_api/discount_coupon'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let data = await autoGetQueryPromotionEnteroriseListPage(
        goodsQuery.page,
        goodsQuery.limit,
    ...
    async getBottomList() {
      ...
      let data = await queryPromotionListPage(
    ...
    async addClick(row) {
      ...
      row.couponActivityAutoGetId = this.couponActivityId
      enterpriseLimitList.push(row)
      let data = await savePromotionEnterpriseLimit(enterpriseLimitList)
      if (data) {
    ...
    async batchAddClick() {
      ...
      this.goodsList.forEach(item => {
        item.couponActivityAutoGetId = this.couponActivityId
    ...
    async removeClick(row) {
      let ids = []
      ids.push(row.id)
      let data = await deletePromotionEnterpriseLimit(this.couponActivityId, ids)
      if (data) {
        this.$common.n_success('保存成功');
        // 如果当前页数据 全部删除，需要回前一页
