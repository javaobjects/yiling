    ...
    <div class="app-container-content">
      <div class="common-box">
        <div class="search-box">
          <el-row class="box">
            <el-col :span="6">
              ...
              <el-input v-model="query.name" placeholder="请输入发券活动名称" @keyup.enter.native="handleSearch" />
            </el-col>
            <el-col :span="6">
              <div class="title">发券活动ID</div>
              <el-input v-model="query.id" placeholder="请输入发券活动ID" @keyup.enter.native="handleSearch" />
            </el-col>
            <el-col :span="12">
        ...
        <div class="search-box mar-t-16">
          <el-row class="box">
            <el-col :span="6">
              <div class="title">发券类型</div>
              <el-select v-model="query.type" placeholder="请选择发券类型">
      ...
      <div class="down-box clearfix">
        <ylButton type="primary" @click="addClick">添加自动发券</ylButton>
      </div>
      <!-- 底部列表 -->
          ...
          <el-table-column label-class-name="mar-l-16" label="优惠券信息" min-width="380" align="left">
            <template slot-scope="{ row }">
              <div class="item text-l mar-l-16">
                ...
                <div class="item-text font-size-base font-title-color"><span>发券活动ID：</span>{{ row.id }}</div>
...
import { 
  couponActivityAutoGive1ueryListPage, 
  queryAutoGiveListPage,
  couponActivityAutoGiveEnable,
  couponActivityAutoGiveStop,
  couponActivityAutoGivesScrap
...
export default {
  ...
  methods: {
    async getList() {
      ...
      let data = await couponActivityAutoGive1ueryListPage(
        query.page,
        query.limit,
        query.name,
        query.id,
    ...
    async getSendList() {
      ...
      let data = await queryAutoGiveListPage(
        sendNumQuery.page,
        sendNumQuery.limit,
        this.currentRow.id
      );
    ...
    async enableClick(row) {
      ...
      let data = await couponActivityAutoGiveEnable(row.id)
    ...
    async stopConfirm() {
      this.$common.showLoad()
      let data = await couponActivityAutoGiveStop(this.currentOperationRow.id)
      this.$common.hideLoad()
      if (typeof data != 'undefined') {
    ...
    async cancellationConfirm() {
      this.$common.showLoad()
      let data = await couponActivityAutoGivesScrap(this.currentOperationRow.id)
      this.$common.hideLoad()
      if (typeof data != 'undefined') {
    ...
    async downLoadTemp() {
      ...
      let data = await createDownLoad({
        className: 'couponActivityAutoHasGiveExportService',
        fileName: '自动发放优惠券已发放导出',
        groupName: '自动发放优惠券',
