        ...
        <div class="search-box mar-t-16">
          <el-row class="box">
            ...
            <el-col :span="6">
              ...
              <el-select v-model="query.strategyType" placeholder="请选择">
                <el-option label="全部" :value="0"></el-option>
                <el-option
      ...
      <div class="mar-t-8 pad-b-10 order-table-view">
        ...
        >
          ...
          <el-table-column label="参与次数" min-width="120" align="center">
            <template slot-scope="{ row }">
              <div>
                <yl-button type="text" @click="useNumClick(row)">{{ row.times }}</yl-button>
              </div>
            </template>
          </el-table-column>
      ...
      <div class="dialog-content-box">
        ...
        <div class="mar-t-10" style="padding-bottom: 10px;background: #FFFFFF;">
          ...
          >
            ...
            <el-table-column label="促销活动ID" min-width="100" align="center" prop="marketingStrategyId">
            </el-table-column>
            <el-table-column label="促销活动名称" min-width="200" align="center" prop="marketingStrategyName">
            </el-table-column>
            ...
            <el-table-column label="生效条件" min-width="200" align="center" prop="condition">
            </el-table-column>
            <el-table-column label="促销结果" min-width="200" align="center">
              <template slot-scope="{ row }">
                <div v-for="(item, index) in row.strategyResult" :key="index">{{ item }}</div>
              </template>
            </el-table-column>
            <el-table-column label="执行结果" min-width="100" align="center" prop="eid">
              <template slot-scope="{ row }">
                <div>
                  {{ row.implementResult == 1 ? '成功' : '失败' }}
                </div>
              </template>
            </el-table-column>
...
import {
  strategyActivityPageList,
  strategyActivityCopy,
  strategyActivityStop,
  strategyActivityRecordPageList
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let data = await strategyActivityPageList(
        query.page,
        query.limit,
        query.name,
        query.status,
    ...
    async copyClick(row) {
      ...
      let data = await strategyActivityCopy(row.id)
    ...
    async sendNumTotalListPageListMethod() {
      ...
      let data = await strategyActivityRecordPageList(
        sendNumQuery.page,
        sendNumQuery.limit,
        this.currentOperationRow.id,
        this.currentOperationRow.strategyType
      );
      this.loading2 = false
      if (data) {
    ...
    async stopConfirm() {
      this.$common.showLoad()
      let data = await strategyActivityStop(this.currentOperationRow.id ,2)
      this.$common.hideLoad()
    ...
    async downLoadTemp() {
      ...
      let data = await createDownLoad({
        className: 'strategyRecordExportService',
        fileName: '导出策略满赠参与次数信息',
        groupName: '策略满赠参与次数信息导出',
        menuName: '策略满赠参与次数信息',
        searchConditionList: [
          {
            desc: '营销活动id',
            name: 'marketingStrategyId',
            value: this.currentOperationRow.id
          },
          {
            desc: '策略类型',
            name: 'strategyType',
            value: this.currentOperationRow.strategyType
          }
        ]
      })
      this.$common.hideLoad()
