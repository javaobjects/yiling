...
import { strategyLimitSellerAdd, strategyLimitSellerDelete, strategyLimitSellerPageList, strategyLimitPromoterMemberAdd, strategyLimitPromoterMemberDelete, strategyLimitPromoterMemberPageList } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    ...
    async getBottomList() {
      ...
      let data = null
      if (this.fromType == 1) {
        data = await strategyLimitSellerPageList(
        ...
        data = await strategyLimitPromoterMemberPageList(
          bottomGoodsQuery.page,
          bottomGoodsQuery.limit,
          this.marketingStrategyId,
          Number(bottomGoodsQuery.eid) || '',
    ...
    async addClick(row) {
      ...
      let data = null
      if (this.fromType == 1) {
        data = await strategyLimitSellerAdd(this.marketingStrategyId, enterpriseLimitList)
      } else if (this.fromType == 2) {
        data = await strategyLimitPromoterMemberAdd(this.marketingStrategyId, enterpriseLimitList)
      }
    ...
    async removeClick(row) {
      ...
      if (this.fromType == 1) {
        ...
        data = await strategyLimitPromoterMemberDelete(this.marketingStrategyId, ids)
      }
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        // 如果当前页数据 全部删除，需要回前一页
