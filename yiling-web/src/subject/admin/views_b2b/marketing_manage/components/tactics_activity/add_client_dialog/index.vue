...
<script>
import { strategyLimitBuyerPage, strategyLimitBuyerPageList, strategyLimitBuyerAdd, strategyLimitBuyerDelete } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    ...
    async getBottomList() {
      ...
      let data = await strategyLimitBuyerPageList(
    ...
    async addClick(row) {
      let eidList = []
      eidList.push(row.id)
      let data = await strategyLimitBuyerAdd(this.marketingStrategyId, eidList)
    ...
    async addSearchClick() {
      ...
      let data = await strategyLimitBuyerAdd(this.marketingStrategyId, undefined, Number(goodsQuery.eid) || '', goodsQuery.ename)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
    ...
    async removeClick(row) {
      let ids = []
      ids.push(row.eid)
      let data = await strategyLimitBuyerDelete(this.marketingStrategyId, ids)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
    ...
    async deleteSearchClick() {
      ...
      let data = await strategyLimitBuyerDelete(this.marketingStrategyId, undefined, Number(bottomGoodsQuery.eid) || '', bottomGoodsQuery.ename)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
