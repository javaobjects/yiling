...
<script>
import { strategyLimitMemberPage, strategyLimitMemberPageList, strategyLimitMemberAdd, strategyLimitMemberDelete } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    ...
    async getBottomList() {
      ...
      let data = await strategyLimitMemberPageList(
    ...
    async addClick(row) {
      let data = await strategyLimitMemberAdd(this.marketingStrategyId, row.id)
    ...
    async removeClick(row) {
      let data = await strategyLimitMemberDelete(this.marketingStrategyId, row.memberId)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
