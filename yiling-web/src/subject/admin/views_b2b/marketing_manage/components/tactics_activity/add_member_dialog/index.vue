...
<script>
import { strategyStageMemberList, strategyStageMemberPageList, strategyStageMemberAdd, strategyStageMemberDelete } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let data = await strategyStageMemberList(
        goodsQuery.page,
        goodsQuery.limit,
        goodsQuery.name
      );
    ...
    async getBottomList() {
      ...
      let data = await strategyStageMemberPageList(
    ...
    async addClick(row) {
      let ids = []
      ids.push(row.id)
      let data = await strategyStageMemberAdd(this.marketingStrategyId, ids)
    ...
    async removeClick(row) {
      ...
      let data = await strategyStageMemberDelete(this.marketingStrategyId, ids)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        // 如果当前页数据 全部删除，需要回前一页
