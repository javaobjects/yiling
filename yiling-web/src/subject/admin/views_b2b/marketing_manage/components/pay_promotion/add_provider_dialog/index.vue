...
import { payPromotionLimitSellerAdd, payPromotionyLimitSellerDelete, payPromotionLimitSellerPageList, payPromptionLimitPromoterMemberAdd, payPromptionLimitPromoterMemberDelete, payPromptionLimitPromoterMemberPageList } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    init(marketingStrategyId, running, operationType, fromType) {
      this.initData()
    ...
    async getBottomList() {
      ...
      let data = null
      if (this.fromType == 1) {
        data = await payPromotionLimitSellerPageList(
        ...
        data = await payPromptionLimitPromoterMemberPageList(
          bottomGoodsQuery.page,
          bottomGoodsQuery.limit,
          this.marketingStrategyId,
          Number(bottomGoodsQuery.eid) || '',
    ...
    async addClick(row) {
      ...
      let data = null
      if (this.fromType == 1) {
        data = await payPromotionLimitSellerAdd(this.marketingStrategyId, enterpriseLimitList)
      } else if (this.fromType == 2) {
        data = await payPromptionLimitPromoterMemberAdd(this.marketingStrategyId, enterpriseLimitList)
      }
    ...
    async removeClick(row) {
      ...
      let data = null
      if (this.fromType == 1) {
        data = await payPromotionyLimitSellerDelete(this.marketingStrategyId, ids)
      } else if (this.fromType == 2) {
        data = await payPromptionLimitPromoterMemberDelete(this.marketingStrategyId, ids)
      }
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        // 如果当前页数据 全部删除，需要回前一页
    ...
    async batchRemoveClick() {
      ...
      let data = null
      if (this.fromType == 1) {
        data = await strategyLimitSellerDelete(this.marketingStrategyId, ids)
      } else if (this.fromType == 2) {
        data = await payPromptionLimitPromoterMemberDelete(this.marketingStrategyId, ids)
      }
      if (typeof data !== 'undefined') {
