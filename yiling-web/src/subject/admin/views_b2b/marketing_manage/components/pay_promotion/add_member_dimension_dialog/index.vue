...
<script>
import { strategyLimitMemberPage, payPromptionLimitMemberPageList, payPromptionLimitMemberAdd, payPromptionLimitMemberDelete } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    ...
    async getBottomList() {
      ...
      let data = await payPromptionLimitMemberPageList(
    ...
    async addClick(row) {
      let data = await payPromptionLimitMemberAdd(this.marketingStrategyId, row.id)
    ...
    async removeClick(row) {
      let data = await payPromptionLimitMemberDelete(this.marketingStrategyId, row.memberId)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
