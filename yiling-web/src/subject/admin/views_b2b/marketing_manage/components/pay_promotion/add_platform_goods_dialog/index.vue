...
<script>
import { strategyLimitPlatformGoodsList, payPromotionLimitPlatformGoodsAdd, payPromotionLimitPlatformGoodsDelete, payPromotionLimitPlatformGoodsPageList } from '@/subject/admin/api/b2b_api/marketing_manage'
...
export default {
  ...
  methods: {
    ...
    async getBottomList() {
      ...
      let data = await payPromotionLimitPlatformGoodsPageList(
    ...
    async addClick(row) {
      ...
      let data = await payPromotionLimitPlatformGoodsAdd(this.marketingStrategyId, sellSpecificationsIdList)
    ...
    async addSearchClick() {
      ...
      let data = await payPromotionLimitPlatformGoodsAdd(this.marketingStrategyId, undefined, Number(goodsQuery.standardId) || '', Number(goodsQuery.sellSpecificationsId) || '', goodsQuery.goodsName, goodsQuery.manufacturer, goodsQuery.isYiLing)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
    ...
    async removeClick(row) {
      ...
      let data = await payPromotionLimitPlatformGoodsDelete(this.marketingStrategyId, ids)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        // 如果当前页数据 全部删除，需要回前一页
    ...
    async deleteSearchClick() {
      ...
      let data = await payPromotionLimitPlatformGoodsDelete(this.marketingStrategyId, undefined, Number(bottomGoodsQuery.standardId) || '', Number(bottomGoodsQuery.sellSpecificationsId) || '', bottomGoodsQuery.goodsName, bottomGoodsQuery.manufacturer, bottomGoodsQuery.isYiLing)
      if (typeof data !== 'undefined') {
        this.$common.n_success('保存成功');
        this.getBottomList()
      } 
