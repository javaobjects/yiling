                  ...
                  <div class="flex1 font-size-base">
                    <div class="product-num">
                      ...
                      <div>
                        ...
                        <span class="font-light-color">商品总数：{{ row.total }}</span>
                      </div>
                    </div>
                  </div>
...
<script>
import { getProductList } from '@/subject/admin/api/b2b_api/products'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      if (data) {
        this.dataList = data.records.map(item => {
          item.total = item.upShelfCount + item.unShelfCount + item.waitSetCount
          return item
        })
    ...
    showDetail(row, index) {
      this.$router.push(`b2b_products_detail/${row.id}/${row.name}`)
    }
  }
}
</script>
