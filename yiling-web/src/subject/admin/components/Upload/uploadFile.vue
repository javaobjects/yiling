    ...
    methods: {
      ...
      async uploadFile(params) {
        ...
        if (this.ossKey) {
          ...
          formData.append('x-oss-meta-filename', params.file.name)
          ...
          const imgData = await axios.post(ossData.host, formData, {
            ...
            onUploadProgress: (event) => {
              if (this.totalFileNum === 1) {
                ...
                this.percent = Number(progress.toFixed(2))
              }
            }
          })
          if (imgData.data.code === 200) {
            this.$emit('onSuccess', Object.assign(imgData.data.data, {
              key: imgData.data.data.object,
      ...
      async getOssKeyId() {
        if (this.ossKey) {
          const ossKey = `yl_oss_ssc_${process.env.NODE_ENV}_${this.ossKey}_${this.callbackUrl || ''}`
          const data = this.$common.getSessionCache(ossKey)
          if (data) {
            return data
          } else {
            const callUrl = this.callbackUrl ? process.env.VUE_APP_REQ_URL + this.callbackUrl : ''
            const data = await getOssKey(this.ossKey, callUrl)
            if (data) {
