<template>
  <div class="app-container">
            ...
            <div class="item-info mar-t-16">
              <el-row>
                <el-col :span="8">
                  ...
                  <div class="item">
                    <div class="item-title">来源终端：<span class="item-value">{{ detailData.ename }}</span></div>
                  </div>
                  <div class="item">
                    <div class="item-title">配送方式：<span class="item-value">{{ detailData.deliverType | dictLabel(hmcDeliverType) }}</span></div>
                  </div>
                  <div class="item">
          ...
          <div class="insurance-detail-item">
            ...
            <div class="item-info mar-t-16">
              <el-row>
                <el-col :span="8">
                  <div class="item">
                    <div class="item-title">注册用户ID：<span class="item-value">{{ detailData.createUser }}</span></div>
                  </div>
                </el-col>
                <el-col :span="8">
                  <div class="item">
                    <div class="item-title">用户昵称：<span class="item-value">{{ detailData.createUserName }}</span></div>
                  </div>
                </el-col>
                <el-col :span="8">
                  <div class="item">
                    <div class="item-title">手机号：<span class="item-value">{{ detailData.createUserPhone }}</span></div>
                  </div>
                </el-col>
              </el-row>
            </div>
              ...
              <el-row>
                ...
                <el-col :span="24">
                  <div class="item mar-t-16">
                    ...
                    <yl-button v-if="ihEid != 1" class="mar-t-16" plain type="primary" @click="addRemark()">{{ detailData.platformRemark ? '修改备注':'添加备注' }}</yl-button>
                  </div>
                </el-col>
                <el-col :span="24">
                  <div class="item mar-t-16">
    ...
    <!-- 添加 修改备注-->
    <yl-dialog :title="remarkTitle" width="800px" :visible.sync="showRemarkDialog" :show-footer="true" @confirm="confirm" class="dialog-updata">
    ...
    <!-- 快递 -->
    <yl-dialog :title="deliverTitle" width="800px" :visible.sync="showdeliverDialog" :show-footer="true" @confirm="confirm1" class="dialog-updata">
      <div class="dialog-content">
...
<script>
import { insuranceGoodsListDetail, editGoodsOrderDelivery, addPlatformRemark } from '@/subject/admin/api/cmp_api/insurance_order_manage'
import {
  hmcOrderStatus,
  hmcPaymentStatus,
  hmcPaymentMethod,
...
export default {
  ...
  methods: {
    async getDetail() {
      let data = await insuranceGoodsListDetail(this.id)
      if (data) {
    ...
    addRemark() {
      if (this.detailData.platformRemark) {
        this.remarkTitle = '修改备注'
        this.ruleForm.remark = this.detailData.platformRemark
      } else {
