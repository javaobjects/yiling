              ...
              <div class="table-view" :key="$index">
                ...
                <div class="content flex-row-left">
                  <div class="table-item">
                    ...
                    <div class="item font-size-base font-title-color"><span class="font-light-color">联系人姓名：</span>{{ row.contactor }}</div>
                    <div class="item font-size-base font-title-color"><span class="font-light-color">联系人电话：</span>{{ row.contactorPhone }}</div>
                  </div>
                  <div class="flex1 font-size-base">
                    <div class="product-num">
                      <div>商品数量</div>
                      <div>
                        <span class="font-light-color">上架中：{{ row.upShelfCount }}</span>
                        <span class="font-light-color">已下架：{{ row.unShelfCount }}</span>
                        <span class="font-light-color">待设置：{{ row.waitSetCount }}</span>
                        <span class="font-light-color">商品总数：{{ (row.upShelfCount || 0) + (row.unShelfCount || 0) + (row.waitSetCount || 0) }}</span>
                      </div>
                    </div>
                  </div>
...
<script>
import { getProductList } from '@/subject/admin/api/products'
  ...
  methods: {
    ...
    async getList() {
      ...
      if (data) {
        this.dataList = data.records.map(item => {
          item.total = item.upShelfCount + item.unShelfCount + item.underReviewCount + item.rejectCount
          return item
        })
    ...
    goImport() {
      this.$router.push({
        ...
        query: {
          action: '/pop/api/v1/admin/goods/import'
        }
      })
    },
    // 查看详情
    showDetail(row, index) {
      this.$router.push(`products_detail/${row.id}/${row.name}`)
    }
  }
}
</script>
