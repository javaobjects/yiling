<template>
  <yl-dialog
    ...
    <div class="used-box">
      ...
      <div class="pad-t-16 pad-lr-16" v-show="totalData.paymentDayType == 1">
      ...
      <div class="desc mar-t-16">
        <span class="first-span"><span >授信主体：</span><span class="font-important-color">{{ totalData.ename }}</span></span>
        ...
        <yl-table
          ...
          <el-table-column min-width="120" align="center" v-if="!isHk" :key="Math.random()">
            <template slot="header">
              <div>已使用额度</div>
              <span>{{ `（总计：${$options.filters.toThousand(totalData.totalUsedAmount)}元）` }}</span>
            </template>
            <template slot-scope="{ row }">
          ...
          <el-table-column min-width="120" align="center" :key="Math.random()">
            <template slot="header">
              <div>订单金额</div>
              <span>{{ `（总计：${$options.filters.toThousand(totalData.orderAmount, '￥')}元）` }}</span>
            </template>
            <template slot-scope="{ row }">
              <span>{{ row.orderAmount | toThousand('￥') }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="totalData.paymentDayType == 2" min-width="120" align="center" :key="Math.random()">
            <template slot="header">
              <div>退款金额</div>
              <span>{{ `（总计：${$options.filters.toThousand(totalData.returnAmount, '￥')}元）` }}</span>
            </template>
            <template slot-scope="{ row }">
          ...
          <el-table-column min-width="120" align="center" v-if="isHk" :key="Math.random()">
            <template slot="header">
              <div>已还款金额</div>
              <span>{{ `（总计：${$options.filters.toThousand(totalData.repaymentAmount, '￥')}元）` }}</span>
            </template>
            <template slot-scope="{ row }">
          ...
          <el-table-column min-width="120" align="center" v-if="isHk" :key="Math.random()">
            <template slot="header">
              <div>待还款金额</div>
              <span>{{ `（总计：${$options.filters.toThousand(totalData.unRepaymentAmount, '￥')}元）` }}</span>
            </template>
            <template slot-scope="{ row }">
          ...
          <el-table-column label="还款时间" min-width="120" align="center" v-if="isHk">
            <template slot-scope="{ row }">
              <div style="padding: 8px 0;">
                <span>{{ row.expirationTime | formatDate }}</span>
              </div>
            </template>
          </el-table-column>
        </yl-table>
...
<script>
  import { getUsedList, getUsedListTotalData } from '@/subject/admin/api/zt_api/period'
  import { repayStatus } from '@/subject/admin/utils/busi'
  ...
  export default {
    ...
    methods: {
      ...
      async getTotal() {
        ...
        if (data) {
          this.totalData = Object.assign({}, data)
        }
      },
      handleSearch() {
        this.query.page = 1
