  ...
  <div class="app-container">
    <div class="app-container-content has-bottom-bar">
      ...
      <div class="common-box">
        ...
        <div class="examine-row">
          <div class="examine-row-title">{{ query.name }}</div>
          <el-row >
            <el-col :span="12">
              <div class="examine-row-conter">
                <p>企业类型：{{ query.type | dictLabel(companyType) }}</p>
                <p>联系人姓名：{{ query.contactor }}</p>
                <p>统一社会信用代码/执业许可证号：{{ query.licenseNumber }}</p>
              </div>
            </el-col>
            <el-col :span="12">
              <div class="examine-row-conter">
                <p>企业ID：{{ query.eid }}</p>
                <p>联系人手机号：{{ query.contactorPhone }}</p>
                <p>企业地址：{{ query.provinceName }}{{ query.cityName }}{{ query.regionName }} {{ query.address }}</p>
              </div>
            </el-col>
          </el-row>
        </div>
      ...
      <div class="common-box">
        ...
        <div class="examine-row">
          <el-form :model="form" :rules="rules" label-position="left" ref="dataForm" label-width="80px" class="demo-ruleForm">
            ...
            <!-- <el-form-item v-show="form.authStatus == '' || form.authStatus == 2" label="驳回原因">
...
<script>
import { enterpriseAuthGet, updateAuth } from '@/subject/admin/api/zt_api/enterprise_audit';
...
export default {
  ...
  methods: {
    async getDetail() {
     let data = await enterpriseAuthGet(
       this.query.eid
     )
     if (data != undefined) {
       this.query = data;
      
       this.qualificationData = data.certificateVOList;
     }
    },
    // 点击图片
    imgClick(val) {
