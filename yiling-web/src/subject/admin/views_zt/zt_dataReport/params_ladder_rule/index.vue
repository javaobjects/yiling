    ...
    <div class="app-container-content">
      <div class="pad-b-100">
        <yl-table border :show-header="true" :list="dataList" :total="query.total" :page.sync="query.page" :limit.sync="query.limit" :loading="loading" @getList="getList">
          ...
          <el-table-column min-width="100" align="center" label="阶梯名称" prop="name"></el-table-column>
...
export default {
  ...
  methods: {
    async getList() {
      ...
      this.loading = true
      let data = await getParamsTypeList(this.query.page, this.query.limit, 3, this.typeID);
      console.log(data);
      this.loading = false
      if (data !== undefined) {
    ...
    addGoods(row) {
      ...
      this.addGoodsTitle = '阶梯规则--' + row.name;
      this.addShow = true;
      this.options1 = []
      this.options = []
    },
    ...
    confirm() {
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          ...
          let data = await saveLadderGoods(
            this.form.id,
            this.form.bussinessId,
            this.form.goodsName,
            this.form.goodsUnit,
