  ...
  <div class="app-container">
    <div class="app-container-content">
        ...
        <div class="search-box">
          <el-row class="box">
            ...
            <el-col :span="8">
              ...
              <el-input v-model="query.activityName" placeholder="请输入" @keyup.enter.native="handleSearch"></el-input>
            </el-col>
          </el-row>
      ...
      <div class="down-box clearfix">
        <div class="btn">
          <el-link class="mar-r-10" type="primary" :underline="false" :href="'NO_12' | template">
            下载导入模板
          </el-link>
          <ylButton type="primary" plain @click="goImport">导入促销活动</ylButton>
        </div>
      </div>
      <div class="mar-t-8 pad-b-100">
        <yl-table border :show-header="true" :list="dataList" :total="query.total" :page.sync="query.page" :limit.sync="query.limit" :loading="loading" @getList="getList">
          ...
          <el-table-column min-width="100" align="center" label="活动名称" prop="activityName"></el-table-column>
          ...
          <el-table-column min-width="100" align="center" label="奖励金额">
            <template slot-scope="{ row }">
              <div>
                <span>
      ...
      <!-- 添加商品 -->
      <yl-dialog :visible.sync="addShow" width="1200px" :title="'促销活动--' + dialogName" @confirm="confirm">
  ...
  methods: {
    async getList() {
      ...
      let data = await getQueryParamSubGoodsPageList(
        ...
        query.limit,
        this.typeId,
        query.activityName,// 活动名称 阶梯中用不到
        query.eid, // 商业名称
        query.updateUser,
        query.goodsName,
      ...
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          ...
          */
          ...
          let data = await updateActivityGoods(
            this.form.id,
    ...
    // 删除
    del(row) {
      this.$common.confirm('确定要删除当前活动吗?', async confirm => {
        if (confirm) {
    ...
    goImport() {
      ...
      let action = ''
      if (typeId == 4) {
        action = '/dataCenter/api/v1/report/param/importSubParFirst'
      } else if (typeId == 5) {
        action = '/dataCenter/api/v1/report/param/importSubParSecond'
      } else if (typeId == 6) {
        action = '/dataCenter/api/v1/report/param/importSubParThird'
      } else if (typeId == 7) {
        action = '/dataCenter/api/v1/report/param/importSubParFourth'
      } else if (typeId == 8) {
        action = '/dataCenter/api/v1/report/param/importSubParFifth'
      } else if (typeId == 9) {
        action = '/dataCenter/api/v1/report/param/importSubParXsy'
      }
      if (!action) {
        this.$common.warn('暂无该促销活动导入')
        return
      }
      this.$router.push({
