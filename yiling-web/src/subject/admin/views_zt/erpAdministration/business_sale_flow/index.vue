  ...
  <div class="app-container">
    <div class="app-container-content">
        ...
        <div class="search-box mar-t-16">
          <el-row class="box">
            <el-col :span="8">
              ...
              <el-input v-model="query.customerName" placeholder="请输入内容" />
            </el-col>
            <el-col :span="8">
      ...
      <div class="down-box clearfix">
        <div class="btn">
          <ylButton type="primary" plain @click="crmDownLoad">导出查询crm结果</ylButton>
...
<script>
import { getErpQuerySaleListPage, queryEnterpriseList, getEnterpriseTagList, queryGoodsSpecification } from '@/subject/admin/api/zt_api/erp_flow';
import { erpSaleFlowSource } from '@/subject/admin/busi/erp/flowDirection';
...
export default {
    ...
    return {
      ...
      pickerData: {
        disabledDate: (time) => {
          ...
          let reduce = this.calcMonths(currentdate, - 5)
          let plus = year.toString() + month.toString()
          return !(timedate < reduce || plus < timedate) || time.getFullYear() < '2020' || time.getTime() > Date.now();
        }
      }
    };
  },
  ...
  mounted() {
    ...
    if (query.name && query.time) {
      ...
      // 获取 年份以及月份 的第一天与最后一天
      let val = query.time.split('-')
      let tim = new Date(val[0], val[1], 0).getDate()
      this.query.createTime = [ query.time + '-01' , query.time + '-' + tim ]
      this.handleSearch();
    }
  },
  activated() {
  ...
  methods: {
    ...
    async crmDownLoad() {
      ...
      this.$common.showLoad()
      let data = await createDownLoad({
        className: 'crmFlowSalePageListExportService',
        fileName: '导出查询crm结果',
        groupName: 'erp流向',
        menuName: '运营后台-企业销售流向',
        searchConditionList: [
          {
            desc: '导出来源',
            name: 'menuSource',
